...
verify_launcher_contracts() {
  ...
  reject_file 'FRESH_DAEMON|--fresh-daemon|TERMINAL_WORKSPACE_FRESH_DAEMON' run-dev.sh "run-dev.sh must not expose a daemon replacement escape hatch"
  ...
  if [[ -f run-native-vte-dev.sh ]]; then
    ...
    reject_file 'FRESH_DAEMON|--fresh-daemon|TERMINAL_WORKSPACE_FRESH_DAEMON' run-native-vte-dev.sh "run-native-vte-dev.sh must not expose a daemon replacement escape hatch"
