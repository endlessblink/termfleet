...
const COMMAND_PATTERN =
  /\b(?:npx\s+)?playwright\s+test\b|\b(?:npm|pnpm|yarn)\s+(?:run\s+)?(?:build|check|lint|test|typecheck|verify:[\w:-]+)\b|\bcargo\s+(?:build|check|test)\b/i;
const MIN_TITLE_CHANGE_MS = 2500;
const MIN_PROGRESS_CHANGE_MS = 450;
const RUNNING_ACTIVITY_MAX_AGE_MS = 60_000;

/**
 * Is this command activity still LIVE, or has the label simply latched?
 *
 * `summarizeCommand` infers completion from transcript TEXT — Playwright needs a
 * "N passed (…)" match, `cargo build` has no completion branch at all — and an agent
 * pane emits no shell-integration end event and no prompt lines, so a finished command
 * can keep `status: "running"` indefinitely. That is how a pane running a git command
 * still displayed "Running Playwright tests" (live report 2026-07-25).
 *
 * `shouldApply` stops refreshing `updatedAt` once the transcript stops changing, so
 * staleness is the reliable signal: a genuinely running command keeps its timestamp
 * fresh, a latched one does not. `Terminal.tsx` already used exactly this rule and was
 * the ONLY place that did — the two components that DRAW the line checked the status
 * alone. This is that rule, in one place, for all three callers.
 */
export function durableActivityIsLive(
  activity: TerminalActivitySummary | null | undefined,
  now: number = Date.now(),
): boolean {
  if (activity?.status !== "running") return false;
  return now - (activity.updatedAt ?? 0) < RUNNING_ACTIVITY_MAX_AGE_MS;
}
...
export function parseShellIntegrationEvents(
  transcript: string,
): ShellIntegrationEvent[] {
  return [...transcript.matchAll(OSC_PATTERN)].flatMap<ShellIntegrationEvent>(
    (match) => {
      const code = match[1];
        ...
        return [
          {
            ...
            command: cleanText(decodeShellValue(body)) ?? cleanText(body),
          },
        ];
      if (marker === "P") {
      ...
      if (marker === "7")
        return [{ kind: "cwd" as const, cwd: cleanText(body) }];
      return [];
    },
  );
}
...
function targetFile(command: string) {
  return normalizeCommand(command).match(
...
function scriptCompletionTitle(title: string, success: boolean) {
  if (/^Checking map terminal source contract$/i.test(title))
    return success
      ? "Map terminal source checks passed"
      : "Map terminal source checks failed";
  if (/^Checking status summary server contract$/i.test(title))
    return success
      ? "Status summary server checks passed"
      : "Status summary server checks failed";
  if (/^Checking terminal summary visual headers$/i.test(title))
    return success
      ? "Terminal summary visual checks passed"
      : "Terminal summary visual checks failed";
  if (/^Building frontend$/i.test(title))
    return success ? "Frontend build passed" : "Frontend build failed";
  if (/^Checking TypeScript$/i.test(title))
    return success ? "TypeScript checks passed" : "TypeScript checks failed";
  if (/^Linting frontend$/i.test(title))
    return success ? "Frontend lint passed" : "Frontend lint failed";
  if (/^Running test suite$/i.test(title))
    return success ? "Test suite passed" : "Test suite failed";
  return undefined;
}
...
function humanizeGrep(value: string) {
  const cleaned = value
    ...
    .trim();
  if (
    /\b(durable activity|stale transcript|header uses durable|summary.*glitch|terminal header)\b/i.test(
      cleaned,
    )
  ) {
    return "terminal header stability";
  }
  if (/\b(login|auth|authentication|sign in)\b/i.test(cleaned))
    return "login flow";
  ...
  return cleaned
    .replace(/^should\s+/i, "")
    .replace(
      /\b(header|summary|terminal|map|split|shell|playwright)\b/gi,
...
function activityForPlaywrightName(value?: string) {
  ...
  if (!text) return null;
  if (
    /map shell header uses durable activity instead of stale transcript summary/i.test(
      text,
    )
  ) {
    return {
...
function currentPlaywrightTestName(transcript: string) {
  const matches = [
    ...transcript.matchAll(
      /(?:\[\d+\/\d+\]\s+)?[\w./-]+\.(?:spec|test)\.(?:tsx?|jsx?):\d+:\d+\s+›\s+(.+)/gi,
    ),
  ];
...
function playwrightProgressSubtitle(transcript: string) {
  const runningMatches = [
    ...transcript.matchAll(
      /Running\s+(\d+)\s+tests?\s+using\s+(\d+)\s+workers?/gi,
    ),
  ];
  const passedMatches = [
    ...transcript.matchAll(/(\d+)\s+passed\s+\(([^)]+)\)/gi),
  ];
  const failedMatches = [
    ...transcript.matchAll(/(\d+)\s+failed(?:\s+\(([^)]+)\))?/gi),
  ];
  ...
  const browser = transcript.match(
    /\b(chromium|firefox|webkit|chrome)\b/i,
  )?.[1];
  if (failed)
    return [`${failed[1]} failed`, failed[2], browser]
      .filter(Boolean)
      .join(" · ");
  if (passed)
    return [`${passed[1]} passed`, passed[2], browser]
      .filter(Boolean)
      .join(" · ");
  if (running) {
...
function playwrightTitle(command: string) {
  const grep =
    quotedFlagValue(command, "-g") ?? quotedFlagValue(command, "--grep");
...
function playwrightSubtitle(command: string, transcript: string) {
  const named = activityForPlaywrightName(
    currentPlaywrightTestName(transcript) ??
      quotedFlagValue(command, "-g") ??
      quotedFlagValue(command, "--grep"),
  );
  const file = targetFile(command)?.split("/").filter(Boolean).pop();
  const fileActivity = activityForPlaywrightFile(file);
  const progress = playwrightProgressSubtitle(transcript);
  return (
    [named?.detail ?? fileActivity?.detail, progress, file]
      .filter(Boolean)
      .join(" · ") || undefined
  );
}
...
function playwrightCompletionTitle(intent: string, success: boolean) {
  ...
  if (/^Checking terminal cards on the map$/i.test(intent))
    return `Map terminal card checks ${outcome}`;
  if (/^Checking activity summary wording$/i.test(intent))
    return `Activity summary checks ${outcome}`;
  if (/^Checking login flow$/i.test(intent))
    return `Login flow checks ${outcome}`;
  if (/^Checking checkout flow$/i.test(intent))
    return `Checkout flow checks ${outcome}`;
  if (/^Verifying\s+(.+)/i.test(intent)) {
    const target = intent.replace(/^Verifying\s+/i, "");
    return success ? `${target} verified` : `${target} failed`;
  }
  if (/^Running Playwright tests$/i.test(intent))
    return `Playwright tests ${outcome}`;
  return `${intent} ${outcome}`;
}

function completionTitle(
  previous: TerminalActivitySummary | undefined,
  success: boolean,
) {
  ...
  if (/^Verifying terminal header stability$/i.test(base)) {
    return success
      ? "Terminal header verification passed"
      : "Terminal header verification failed";
  }
  if (/^Verifying /i.test(base)) {
    return success
      ? `${base.replace(/^Verifying /i, "")} verified`
      : `${base} failed`;
  }
...
function summarizeCommand(
  ...
  const script = command.match(
    /\b(?:npm|pnpm|yarn)\s+(?:run\s+)?([^\s]+)/i,
  )?.[1];
  if (script) {
    const summary = scriptSummary(script);
    const failed = /\b(?:failed|error)\b/i.test(transcript);
    const passed =
      !failed && /\b(?:passed|_OK|checks passed|built in)\b/i.test(transcript);
    return {
      title: passed
        ? summary.passedTitle
        : failed
  ...
  const cargo = command
    .match(/\bcargo\s+(build|check|test)\b/i)?.[1]
    ?.toLowerCase();
  if (cargo) {
    return {
      title:
...
function canChangeTitle(
  previous: TerminalActivitySummary | undefined,
  next: TerminalActivitySummary,
  now: number,
) {
...
function shouldApply(
  ...
  if (previous.subtitle !== next.subtitle)
    return now - previous.updatedAt >= MIN_PROGRESS_CHANGE_MS;
  if (previous.progress !== next.progress)
    return now - previous.updatedAt >= MIN_PROGRESS_CHANGE_MS;
  return false;
}

export function deriveTerminalActivity(
  ...
  if (
    ...
    const next: TerminalActivitySummary = {
      title:
        lastEvent.title ?? input.previous?.title ?? "Running terminal command",
      subtitle: lastEvent.subtitle ?? input.previous?.subtitle,
      progress: lastEvent.progress ?? input.previous?.progress,
      status:
        input.previous?.status === "error" ||
        input.previous?.status === "success"
    ...
    return shouldApply(input.previous, next, now)
      ? next
      : (input.previous ?? next);
  }
  if (lastEvent?.kind === "end") {
    ...
    return {
      ...
      subtitle:
        typeof lastEvent.exitCode === "number"
          ? `exit ${lastEvent.exitCode}`
          : input.previous?.subtitle,
  ...
  const command =
    [...events]
      .reverse()
      .find((event) => event.kind === "command" && event.command)?.command ??
    lastCommandLine(transcriptLines(input.transcript));
  if (command) {
    const next = summarizeCommand(command, input.transcript, now);
    next.startedAt =
      input.previous?.command === command ? input.previous.startedAt : now;
    if (input.runtimeStatus === "exited" && next.status === "running")
      next.status = "success";
    if (input.runtimeStatus === "failed" && next.status === "running")
      next.status = "error";
    return shouldApply(input.previous, next, now)
      ? next
      : (input.previous ?? next);
