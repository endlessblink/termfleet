...
// A temporary status poll must not erase a pane's last accepted purpose. The key is
// pane-owned, so one terminal can never borrow another terminal's Goal.
const lastKnownPaneGoal = new Map<string, string>();
...
function taskSearchRank(task: MasterPlanTask, query: string) {
  ...
  if (task.title.toLowerCase().startsWith(value))
    return -1000 + TASK_STATUS_RANK[task.status];
...
function visibleTaskOptions(tasks: MasterPlanTask[], query: string) {
  ...
  return tasks
    .filter(
      (task) =>
        !normalizedQuery ||
        normalizedTaskSearchText(task).includes(normalizedQuery),
    )
    .sort((a, b) => {
      const byRank =
        taskSearchRank(a, normalizedQuery) - taskSearchRank(b, normalizedQuery);
...
const MAP_TOOLBAR_OPEN_KEY = "termfleet.mapToolbar.open";
const SELECTION_TOOLBAR_MIN_LEFT = 132;
const SELECTION_TOOLBAR_MIN_TOP = 58;
// Tallest terminal-card header layout, in px: workspace kicker row (20) + gap
...
const styles: Record<string, CSSProperties> = {
  ...
  toolbar: {
    ...
    gap: 2,
    padding: 4,
    // Translucent + blurred so the map grid and any node text underneath stay
  ...
  node: {
    ...
    overflow: "visible",
    transition:
      "border-color var(--motion-med), box-shadow var(--motion-med), transform var(--motion-fast)",
  },
  nodeHeader: {
    minHeight: 42,
    display: "flex",
  ...
  terminalStatusBlock: {
    ...
    // and every such resize reflows/clips the PTY grid - that is the map card
    // "jumping" while its text "jiggles".
    minHeight: TERMINAL_STATUS_BLOCK_MIN_HEIGHT,
  },
  ...
  projectEmojiZoomBadge: {
    ...
    pointerEvents: "none",
    boxShadow:
      "0 10px 28px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.05)",
  },
  taskBadge: {
    height: 22,
    minWidth: 0,
  ...
  taskPickerRowActive: {
    border:
      "1px solid color-mix(in srgb, var(--accent-live) 48%, var(--border-subtle))",
    background:
      "color-mix(in srgb, var(--accent-live) 10%, var(--surface-base))",
  },
  taskPickerTaskId: {
    color: "var(--text-primary)",
  ...
  taskPickerPrimaryButton: {
    border:
      "1px solid color-mix(in srgb, var(--accent-live) 52%, var(--border-subtle))",
    background:
      "color-mix(in srgb, var(--accent-live) 13%, var(--surface-base))",
    color: "var(--accent-live)",
  },
  taskPickerDangerButton: {
    border:
      "1px solid color-mix(in srgb, var(--accent-danger) 42%, var(--border-subtle))",
  ...
  noteBody: {
    ...
    padding: 8,
    background:
      "color-mix(in srgb, var(--surface-raised) 88%, var(--surface-base))",
  },
  noteTextarea: {
    width: "100%",
    minHeight: 0,
  ...
  terminalBodyTaskMarker: {
    ...
    borderRadius: 2,
    border:
      "1px solid color-mix(in srgb, var(--accent-live) 70%, var(--border-subtle))",
      ...
      >
        ...
        {stats.total > 0 ? (
          <>
            ...
            <span style={styles.terminalBodyTaskRailMeta}>
              {stats.open} open
            </span>
            <span style={styles.terminalBodyTaskRailMeta}>
              {stats.done} done
            ...
            return (
              ...
              >
                ...
                <span style={{ minWidth: 0 }}>
                  ...
                  <div
                    style={{
                      ...
                      color: done
                        ...
                        : styles.terminalBodyTaskTitle.color,
                      textDecoration: done ? "line-through" : "none",
                      textDecorationThickness: done ? 1 : undefined,
                      textDecorationColor: done
                        ? "color-mix(in srgb, var(--text-tertiary) 48%, transparent)"
                        : undefined,
                    }}
                  >
                    {task.task}
                  ...
                  >
                    <span data-testid={`${testIdPrefix}-task-state`}>
                      {task.state}
                    </span>
                    <span style={{ color: "var(--text-tertiary)" }}>·</span>
                    <span
                      style={styles.agentTaskNext}
                      data-testid={`${testIdPrefix}-task-next`}
                    >
                      Next: {task.next}
                    </span>
                  </div>
...
function TerminalMapPreview({
  ...
  const ageSeconds = preview
    ? Math.max(0, Math.round((Date.now() - preview.updatedAt) / 1000))
    : null;
  ...
  return (
    ...
    >
      ...
      <div style={styles.terminalMapPreviewFooter}>
        <span>
          {preview
            ? `${preview.snapshot.cols}x${preview.snapshot.rows}`
            : "waiting"}
        </span>
        <span>
          {ageSeconds === null ? `${ptyCount} PTY` : `${ageSeconds}s ago`}
        </span>
      </div>
    </div>
  );
...
function CanvasNodeViewImpl({
  ...
  const storedSelectedNodeIds = useWorkspaceStore(
    (state) => state.canvasState.selectedNodeIds,
  );
  const zoom = useWorkspaceStore((state) => state.canvasState.viewport.zoom);
  const viewportX = useWorkspaceStore((state) => state.canvasState.viewport.x);
  const viewportY = useWorkspaceStore((state) => state.canvasState.viewport.y);
  ...
  const updatePreviewPaneUrl = useWorkspaceStore(
    (state) => state.updatePreviewPaneUrl,
  );
  ...
  const setActiveTerminal = useWorkspaceStore(
  ...
  const terminalRendererMode = useWorkspaceStore(
    (state) => state.workspaceUiState.terminalRendererMode,
  );
  ...
  const syncTerminalOverlayBounds = useCallback(() => {
    ...
    // Quantise to whole pixels: the card is CSS-scaled, so raw rects carry
    // sub-pixel noise that would re-place (and re-measure) the overlay on every
    // frame - a resize storm the terminal renders as jitter.
    const next: TerminalOverlayBounds = {
      left: Math.round(bodyRect.left - rootRect.left),
      top: Math.round(bodyRect.top - rootRect.top),
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "termfleet-map-terminal-overlay-sync",
  ...
  const onMouseDown = useCallback(
    (event: React.MouseEvent) => {
      ...
      const currentSelectedNodeIds =
        currentCanvasState.selectedNodeIds ??
        (currentCanvasState.selectedNodeId
          ? [currentCanvasState.selectedNodeId]
          : []);
      ...
      const dragIds =
        currentSelectedNodeIds.includes(node.id) &&
        currentSelectedNodeIds.length > 1
      ...
      function onMouseMove(moveEvent: MouseEvent) {
        ...
        if (!drag) return;
        if (
          Math.abs(moveEvent.clientX - drag.x) > 3 ||
          Math.abs(moveEvent.clientY - drag.y) > 3
        ) {
          moved = true;
        }
  ...
  const onResizeMouseDown = useCallback(
    (event: React.MouseEvent, direction: ResizeDirection) => {
      ...
      function onMouseMove(moveEvent: MouseEvent) {
        ...
        const rawWidth =
          resize.width +
          (affectsEast ? deltaX : 0) -
          (affectsWest ? deltaX : 0);
        const rawHeight =
          resize.height +
          (affectsSouth ? deltaY : 0) -
          (affectsNorth ? deltaY : 0);
        ...
        updateCanvasNode(node.id, {
          ...
          x: snapTerminalPixel(
            affectsWest
              ? resize.nodeX + resize.width - next.width
              : resize.nodeX,
            node.type,
            zoom,
          ),
          y: snapTerminalPixel(
            affectsNorth
              ? resize.nodeY + resize.height - next.height
              : resize.nodeY,
            node.type,
            zoom,
          ),
  ...
  const restoredNodeCwd =
    node.terminalCwd ?? (node as CanvasNode & { cwd?: string }).cwd;
  const restoredNodePaneId = node.id.startsWith("recovered-pane-")
    ? node.id.slice("recovered-pane-".length)
    : node.id;
  const restoredNodePaneIds = new Set([
    node.id,
    restoredNodePaneId,
  ...
  const linkedTab = useWorkspaceStore((state) =>
    state.tabs.find(
      (tab) =>
        tab.terminals.some((terminal) => restoredNodePaneIds.has(terminal.paneId)),
    ) ??
    (node.terminalTabId
      ? state.tabs.find((tab) => tab.id === node.terminalTabId)
      : undefined)
  );
  const panelColor = labelColor ?? linkedTab?.color;
  ...
  const terminalRoot = restoredNodeCwd ?? linkedTab?.initialCwd;
  const taskRoot =
    linkedProject?.projectRoot ??
    terminalRoot ??
    workstream?.gitRoot ??
    workstream?.cwd ??
  ...
  const rootTasks = normalizedTaskRoot
    ? (tasksByRoot[normalizedTaskRoot] ?? [])
    : [];
  const taskOptions = useMemo(
    () => visibleTaskOptions(rootTasks, taskPickerQuery),
    [rootTasks, taskPickerQuery],
  );
  const taskPlanPath = normalizedTaskRoot
    ? masterPlanPath(normalizedTaskRoot)
    : "No project root";
  ...
  const activateTerminalNode = useCallback(() => {
    ...
    if (node.type === "terminal") {
      ...
      setActiveTerminal(
        linkedTerminalId ?? `terminal-${terminalTabId}-${terminalPaneId}`,
      );
    }
  ...
  // Agent panes receive their authoritative sidecar summary on the tab workstream;
  // shell panes keep it on the terminal. The map must read the same pane-owned
  // source as SplitPane or it renders "Goal not captured" after a successful poll.
  const terminalStatusSummaryCandidates = [
    linkedTerminal?.statusSummary,
    linkedTab?.terminals.length === 1 ? workstream?.statusSummary : undefined,
  ].filter((summary): summary is WorkstreamStatusSummary => Boolean(summary));
  const capturedSummaryGoalAccepted = (summary: WorkstreamStatusSummary) => {
    const goal = summary.mainTask?.trim() ?? "";
    const qualityInput = summary.mainTaskSource === "opening-request" && goal.endsWith("?")
      ? `${goal.slice(0, -1)}.`
      : goal;
    if (
  ...
  const purposeTaskLineup = visibleTaskLineup(
    linkedTerminal?.taskLineup ?? workstream?.taskLineup,
  ...
  const terminalVisibleText =
    previewVisibleText &&
    (!linkedTerminal?.terminalVisibleTextUpdatedAt ||
      (terminalPreview?.updatedAt ?? 0) >=
        linkedTerminal.terminalVisibleTextUpdatedAt)
      ? previewVisibleText
      : (linkedTerminal?.terminalVisibleText ?? previewVisibleText);
  const terminalPurposeOutput =
    terminalVisibleText || linkedTerminal?.terminalOutput || "";
  ...
  const terminalOutputClosedRaw = terminalOutputClosesTaskLineup(
  ...
  const terminalAtReadyPrompt = terminalTextLooksReadyPrompt(
    linkedTerminal?.terminalVisibleText,
  );
  // A command label that stopped being refreshed has latched, not carried on running —
  // see durableActivityIsLive.
  const terminalActivityLive = durableActivityIsLive(
    linkedTerminal?.durableActivity,
  );
  const terminalHasConcreteVisibleSummary = Boolean(
    ...
    /^(?:Playwright test|Running daily regression hunt|Running Yahav scrape)$/i.test(
      terminalExtractedSummary.task,
    ),
  );
  const terminalDurableActivityUsable = Boolean(
    linkedTerminal?.durableActivity && terminalActivityLive,
  );
  ...
  const terminalReadyPromptCloses =
    ...
    !terminalHasConcreteVisibleSummary &&
    !purposeTaskLineup.some((item) => item.source === "todo-write");
  const terminalClosedSummary: WorkstreamStatusSummary | null =
    terminalOutputClosed || terminalReadyPromptCloses
      ? {
          ...(terminalExtractedSummary ?? {}),
          task: "Idle",
          path:
            liveTerminalRoot ??
            pathTail(liveTerminalRoot) ??
  ...
  const terminalNeutralTitle =
    terminalDisplaySummaryBase.status === "working"
      ...
      : terminalDisplaySummaryBase.status === "blocked"
        ? "Needs attention"
        : terminalDisplaySummaryBase.status === "done" ||
      ...
      ? [
          {
            ...
            status:
              directlyBoundTask.status === "done" ? "completed" : "in_progress",
            source: "todo-write",
            updatedAt: 0,
          },
        ]
      : (linkedTerminal?.taskLineup ?? workstream?.taskLineup);
  const terminalHeaderTaskLine = linkedTerminal
    ? linkedTerminal.taskLine
    : workstream?.taskLine;
  ...
  const terminalHeaderTaskCandidate = terminalHeaderTaskLineup
    ?.find(
      (item) =>
        item.status === "in_progress" &&
        !/^(?:Working|Thinking|Ready|Ready for next task|Idle|Status unavailable|Activity not captured)$/i.test(
          item.content.trim(),
        ),
    )
    ?.content?.trim();
  const terminalHeaderTaskLineCandidate =
    terminalHeaderTaskLine?.text?.trim() &&
    !/^(?:Working|Thinking|Ready|Ready for next task|Idle|Status unavailable|Activity not captured)$/i.test(
      terminalHeaderTaskLine.text.trim(),
    )
      ? terminalHeaderTaskLine.text.trim()
      : undefined;
  const terminalHeaderDurableActivityTask =
    linkedTerminal?.durableActivity?.title?.trim() &&
    !/^(?:Working|Thinking|Ready|Ready for next task|Idle|Status unavailable|Activity not captured)$/i.test(
      linkedTerminal.durableActivity.title.trim(),
    )
      ? linkedTerminal.durableActivity.title.trim()
      : undefined;
  const terminalHeaderStatusTask =
  ...
  const stabilizedTerminalHeaderTask = stableHeader(
    ...
    { nowMs: Date.now(), holdPlaceholders: true },
  ).title || terminalHeaderTaskDescription || canvasTaskFallback;
  const isCompleteGoalText = (value?: string | null) =>
    Boolean(
      value?.trim() &&
        !/[…]$/.test(value.trim()) &&
  ...
  const terminalHeaderContextCandidate =
    ...
    qualityCheckGoalLabel(terminalHeader.contextLabel, {
       allowAboutWhatVoice:
        ...
        /^(?:this\s+session\s+is\s+about|I['’]m\s+|We['’]re\s+)/i.test(
  ...
  const terminalHeaderTitleRaw = terminalDurableActivityUsable &&
    ...
    ? terminalDisplaySummaryBase.task
    : /^(?:Terminal|Activity not captured|Status unavailable|Idle|Working|Thinking|Ready|Ready for next task|Awaiting next action)$/i.test(
          terminalHeader.currentActivity.trim(),
        )
      ? terminalHeaderContextDescription || canvasGoalFallback
      : terminalHeader.currentActivity;
  const restoredNow = terminalDisplaySummaryBase.now;
  const restoredNowIsDistinct = Boolean(
    restoredNow &&
      !headerTextsEquivalent(restoredNow, terminalHeaderTaskDescription) &&
      !headerTextsEquivalent(restoredNow, canvasGoalFallback),
  );
  const restoredNowIsSettled = Boolean(
      terminalDisplaySummaryBase.narration &&
      restoredNow &&
      restoredNowIsDistinct &&
      qualityCheckNowLabel(restoredNow).ok &&
      !/^(?:Idle|Working|Ready|Awaiting next action|Activity not captured|Status unavailable)$/i.test(
        restoredNow.trim(),
      ),
  );
  const terminalHeaderNowRaw =
    ...
    qualityCheckNowLabel(restoredNow).ok
      ? restoredNow
    : headerTextsEquivalent(terminalHeader.currentActivity, terminalHeaderTaskDescription) ||
      headerTextsEquivalent(terminalHeader.currentActivity, canvasGoalFallback)
  ...
  const stabilizedNodeHeader = stableHeader(
    ...
    {
      title: truncateAtWordBoundary((terminalHeaderTitleRaw ?? "").toString(), 64),
  ...
  const terminalHeaderHasUsefulSummary =
  ...
  const terminalHeaderNowRowText = (() => {
    const acceptableNow = (value?: string | null) => {
      const text = value?.trim() ?? "";
      return text && qualityCheckNowLabel(text).ok ? text : "";
    };
    ...
    const resolved = acceptableNow(linkedTerminal?.nowLine?.text);
    ...
    if (
      ...
      !headerTextsEquivalent(terminalHeaderTitle, terminalHeaderContextDescription) &&
        !headerTextsEquivalent(terminalHeaderTitle, canvasGoalFallback)
    )
      return acceptableNow(terminalHeaderTitle);
    const candidate = acceptableNow(terminalHeaderNow);
    if (!candidate) return "";
    ...
    if (
      headerTextsEquivalent(distinctCandidate, terminalHeaderContextDescription) ||
      headerTextsEquivalent(distinctCandidate, canvasGoalFallback)
    )
      return "";
    return distinctCandidate;
  })();
  const stabilizedTerminalHeaderNowRow = stableHeader(
    `map-now-row:${terminalTabId}:${terminalPaneId}:${node.taskBinding?.taskId ?? "unbound"}`,
    {
  ...
  const boundTask =
    ...
    (detectedLaneTaskId
      ? rootTasks.find(
          (task) => task.id.toLowerCase() === detectedLaneTaskId.toLowerCase(),
        )
      : undefined);
  const queuedWorkstreamInput = workstream?.inputQueue?.find(
    (input) => !input.sentAt,
  );
  const latestInput =
    workstream?.inputQueue?.[workstream.inputQueue.length - 1];
  const latestMissionControlInput =
    latestInput?.source === "mission-control" ? latestInput : undefined;
  ...
  const canDraftStatusCheck =
    workstream?.kind === "agent" && isStaleAgentWorkstream(workstream);
  ...
  const agentMissionTask = linkedTerminal
    ? undefined
    : agentHeaderTitle?.trim() || undefined;
  const agentCardStatusSummary = linkedTerminal
    ? terminalStatusSummary
    : agentStatusSummary;
  const agentCardTask =
    [stabilizedTerminalHeaderTask, agentMissionTask, agentCardStatusSummary?.task?.trim()]
      .find(
        (candidate) =>
          Boolean(candidate) &&
  ...
  const agentCardNow = (() => {
    if (terminalHeaderNowRowStableText) return terminalHeaderNowRowStableText;
    const renderedHeaderNow =
      qualityCheckNowLabel(terminalHeader.currentActivity).ok
        ? terminalHeader.currentActivity
        : undefined;
    const restoredAgentNow =
      resolveDistinctHeaderNow(agentCardTask, renderedHeaderNow) ??
      resolveDistinctHeaderNow(agentCardTask, agentCardStatusSummary?.now);
    if (!restoredAgentNow) {
    ...
    if (
      ...
      return restoredAgentNow;
    return restoredAgentNow;
  })();
  const agentStatusChip =
    workstream?.kind === "agent" && agentStatusSummary
      ? agentStatusChipText(workstream, agentStatusSummary)
      : undefined;
  const laneChecklistTasks = (boundTask?.checklist ?? []).map(
    (item, index) => ({
      id: item.id,
      task: item.text,
  ...
  const currentLineupTasks = currentLineupTaskRows(
    workstream,
    canonicalWorkstreamTaskLineup?.length
      ? canonicalWorkstreamTaskLineup
      : canonicalTerminalTaskLineup,
    workstream?.kind === "agent"
      ? (agentStatusSummary ?? undefined)
      : undefined,
  );
  const terminalBodyTasks =
    laneChecklistTasks.length > 0 ? laneChecklistTasks : currentLineupTasks;
  const taskSidebarCollapsed =
    linkedTerminal?.taskSidebarCollapsed ??
    node.taskSidebarCollapsed ??
    terminalBodyTasks.length === 0;
  const terminalBodyTaskPrefix: "canvas-agent" | "canvas-terminal" =
    isAgentTerminal ? "canvas-agent" : "canvas-terminal";
  const nodeKind =
    isAgentTerminal
      ? "agent"
      : node.type === "terminal"
  ...
  const connectLinkedTerminal = useCallback(() => {
    ...
    if (canReconnectSavedAgent) {
      void reconnectSavedAgentPanes(
        [{ ...currentTab, terminals: [currentTerminal] }],
        [],
        )
        .then((result) => {
          // Re-read after recovery because a verified owner rebind changes the
          // terminal id while preserving this pane id. The previous code kept
          // `targetTerminalId`, which was the idle placeholder shown in the dock.
          const reboundTab = useWorkspaceStore
            .getState()
            .tabs.find((tab) => tab.id === currentTab.id);
          const reboundTerminal = reboundTab?.terminals.find(
            (terminal) => terminal.paneId === targetPaneId,
          );
          attachedTerminalId = reboundTerminal?.id ?? targetTerminalId;
          setConnectedTerminalId(attachedTerminalId);
          setActiveTerminal(attachedTerminalId);
          setConnectionGeneration((generation) => generation + 1);
          ...
          traceTerminalLatency("frontend.map.connect.reconnect", {
            ...
            pendingColdRestore: result.pendingColdRestore.length,
          ...
          } else if (result.pendingColdRestore.length) {
            setCanvasNotice(
              "That chat is saved but no longer running — it comes back on the next relaunch.",
            );
    ...
    const focusConnectedInput = () => {
      ...
      const input =
        inputs.find(
          (candidate) => candidate.dataset.terminalSessionId === attachedTerminalId,
        ) ?? null;
      if (input) {
        useWorkspaceStore
          .getState()
          .setActiveTerminal(input.dataset.terminalSessionId ?? attachedTerminalId);
  ...
  const bindTaskId = useCallback(
    (taskId: string) => {
      ...
      updateCanvasNode(node.id, {
        taskBinding: {
          taskId: trimmed,
          planPath: masterPlanPath(normalizedTaskRoot),
        },
      });
      setTaskPickerOpen(false);
    },
  ...
  const queueOperatorDraft = useCallback(() => {
    ...
    const queued = useWorkspaceStore
      .getState()
      .queueWorkstreamInput(linkedTab.id, operatorDraft);
    if (!queued) return;
    setOperatorDraft("");
  ...
  const onSubmitWorkstreamInput = useCallback(
  ...
  const onDraftRecoveryPrompt = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  const onDraftProofRequest = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  const onDraftStatusCheck = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onCopyWorkstreamBrief = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onStopWorkstream = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onInterruptWorkstream = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onRestartWorkstream = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onReviewWorkstream = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onRequestWorktreeCleanup = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const onExecuteWorktreeCleanup = useCallback(
    (event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      if (!linkedTab?.workstream) return;
  ...
  const liveTerminalComponent = shouldMountTerminal ? (
    <TerminalComponent
      ...
      standalone
      renderScale={
        shouldOverlayTerminal ? 1 : mapTerminalRenderScaleForZoom(zoom)
      }
  ...
  const body =
    node.type === "terminal" && shouldUseNativeSplitForInteraction ? (
      ...
      >
        <div style={styles.nativeTerminalPreviewGrid}>
          <div style={styles.nativeTerminalPrompt}>
            ...
            <span style={styles.nativeTerminalPath}>
              {pathTail(liveTerminalRoot)}
            </span>
          </div>
          <div style={styles.nativeTerminalPrompt}>
      ...
      <textarea
        ...
        onWheel={(event) => event.stopPropagation()}
        onChange={(event) =>
          updateCanvasNode(node.id, { content: event.target.value })
        }
  ...
  return (
    <section
      data-testid={
        node.type === "terminal" ? "canvas-terminal-node" : "canvas-node"
      }
      ...
      style={{
        ...
        height: node.height,
        zIndex:
          node.type === "terminal" && isAgentTerminal
            ? 25
            : selected
              ? 15
              : 1,
      ...
      <div
        ...
        style={{
          ...
          ...(node.type === "terminal" && !agentStatusSummary
            ...
            : null),
          ...(agentStatusSummary ? styles.agentNodeHeader : null),
        }}
        onMouseDown={(event) => {
          if (event.button === 1) {
            onPanStart(event);
      ...
      >
        ...
        {node.type === "terminal" &&
          ...
          !agentStatusSummary && (
            <span
              ...
              title={[
                workstream.startupCommand
                  ? `Starts ${workstream.startupCommand}`
                  : (agentCardTask ?? canvasTaskFallback),
        ...
        {node.type === "terminal" && node.taskBinding && (
          <button
            ...
            style={styles.taskBadge}
            title={
              boundTask ? boundTask.title : "Task not found in MASTER_PLAN.md"
            }
            onMouseDown={(event) => event.stopPropagation()}
            onClick={onBindTask}
          >
        ...
        {node.type === "terminal" && agentStatusSummary ? (
          <div
            ...
            dir="auto"
            title={`${workspaceLabel} · ${agentCardTask} · ${agentStatusSummary.path} · ${agentCardNow}`}
          ...
          >
            ...
            <div
              ...
              data-testid="canvas-agent-working-on"
              data-row-kind="task"
            >
              <span style={styles.agentStatusLabel}>Task</span>
            ...
            <div style={styles.agentStatusDetailGrid}>
              <div
                ...
                data-testid="canvas-agent-status-path"
              >
            ...
            <div
              style={styles.agentStatusChips}
              data-testid="canvas-agent-status-chips"
            >
              <span style={styles.agentStatusChip}>{agentStatusChip}</span>
              {agentStatusSummary.confidence && (
                <span style={styles.agentStatusChip}>
                  summary · {agentStatusSummary.confidence}
                </span>
              )}
            </div>
          </div>
        ) : node.type === "terminal" && (!isAgentTerminal || !agentStatusSummary) ? (
          <div
          ...
          >
              ...
              {(
                <CockpitSnapshotProbe
                  ...
                  entry={{
                    ...
                    tabId: terminalTabId,
                    groupId: linkedTab?.groupId ?? null,
                    ...
                    durableActivityTitle:
                      linkedTerminal?.durableActivity?.title,
                    currentActivity: linkedTerminal?.currentActivity,
                    terminalOutput:
                      linkedTerminal?.terminalOutput?.slice(-1800),
                    terminalVisibleText: terminalVisibleText?.slice(-1800),
                    terminalVisibleTextUpdatedAt:
                      terminalPreview?.updatedAt ??
                      linkedTerminal?.terminalVisibleTextUpdatedAt,
                    statusSummarySource: terminalStatusSummaryIsWorkstream
                      ? workstream?.statusSummarySource
                      : linkedTerminal?.statusSummarySource,
                    statusSummaryError: terminalStatusSummaryIsWorkstream
                      ? workstream?.statusSummaryError
                      : linkedTerminal?.statusSummaryError,
                    statusSummaryUpdatedAt: terminalStatusSummaryIsWorkstream
                      ? workstream?.statusSummaryUpdatedAt
                      : linkedTerminal?.statusSummaryUpdatedAt,
                    statusSummaryNarration: (
                    ...
                    taskLineup: canonicalTerminalTaskLineup.map((item) => ({
                      ...
                      previewVisibleTextUsed:
                        terminalVisibleText === previewVisibleText,
                    },
                  }}
                />
              )}
            ...
            <div
              style={
                terminalHeaderHasTrustedSummary
                  ? styles.terminalStatusLayout
                  : styles.terminalStatusSummaryColumn
              }
            >
          ...
          >
            <div
              ...
              data-testid={
                workstream?.kind === "agent"
                  ? "canvas-agent-node-header-title"
                  : "canvas-node-header-title"
              }
            >
              {renaming ? (
                renameEditor
            ...
            {node.type === "terminal" && (
              <div
                ...
                data-testid={
                  workstream?.kind === "agent"
                    ? "canvas-agent-node-header-meta"
                    : "canvas-node-header-meta"
                }
              >
                {agentHeaderMeta ??
                  (linkedProject
                    ? `${pathTail(liveTerminalRoot)} · ${linkedTab?.title ?? node.title}`
                    : pathTail(liveTerminalRoot))}
              </div>
            )}
        ...
        {node.type === "preview" && (
          <button
            ...
            onClick={(event) => {
              ...
              if (linkedTab) {
                setActiveTab(linkedTab.id);
                if (node.previewPaneId)
                  useWorkspaceStore
                    .getState()
                    .setActivePane(linkedTab.id, node.previewPaneId);
                setWorkspaceMode("split");
              }
            }}
          >
        ...
        {node.type === "terminal" && (
          <div style={styles.agentHeaderActions}>
              ...
              <>
                ...
                <button
                  style={{
                    ...
                    cursor: canReviewWorkstream
                      ? styles.headerButton.cursor
                      : "default",
                  }}
                  title={
                    canReviewWorkstream
                      ? "Mark run reviewed"
                      : "Proof and handoff memory are required before review"
                  }
                  ...
                  <>
                    <button
                      style={{
                        ...
                        cursor: canRequestWorktreeCleanup
                          ? styles.headerButton.cursor
                          : "default",
                      }}
                      title={
                        canRequestWorktreeCleanup
                          ? "Request worktree cleanup"
                          : "Worktree cleanup already requested"
                      }
                    ...
                    <button
                      style={{
                        ...
                        cursor: canExecuteWorktreeCleanup
                          ? styles.headerButton.cursor
                          : "default",
                      }}
                      title={
                        canExecuteWorktreeCleanup
            ...
            <button
              ...
              aria-label={
                linkedTab ? `Close ${linkedTab.title}` : "Remove node"
              }
              onMouseDown={(event) => event.stopPropagation()}
              onClick={(event) => {
                event.stopPropagation();
          ...
          <>
            ...
            >
                    ...
                    {node.taskBinding && (
                      <span
                        ...
                        title={
                          boundTask
                            ? boundTask.title
                            : "Current binding is not present in parsed tasks"
                        }
                      >
              ...
              <div style={styles.taskPickerSearchRow}>
                <label style={styles.taskPickerSearch}>
                  ...
                  <input
                    ...
                    autoFocus
                    onChange={(event) =>
                      setTaskPickerQuery(event.currentTarget.value)
                    }
                  />
                </label>
              ...
              <div
                ...
                data-testid="task-binding-options"
              >
                {!normalizedTaskRoot ? (
                  <div style={styles.taskPickerEmpty}>
                    This terminal does not have a project root yet. Open it from
                    a project workspace before binding a plan task.
                  </div>
                ) : rootTasks.length === 0 ? (
                  <div style={styles.taskPickerEmpty}>
                    No tasks were parsed from MASTER_PLAN.md. Use the manual
                    task id field below if the task exists in another ledger
                    format.
                  </div>
                ) : taskOptions.length === 0 ? (
                  <div style={styles.taskPickerEmpty}>
                    No tasks match this search. Try a task id, status, or a
                    shorter phrase.
                  </div>
                ) : (
                  taskOptions.map((task) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span style={{ minWidth: 0 }}>
                          <span style={styles.taskPickerTaskTitle}>
                            {task.title}
                          </span>
              ...
              <div style={styles.taskPickerFooter}>
                <label style={styles.taskPickerManualBlock}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setManualTaskId(event.currentTarget.value)
                    }
                    onKeyDown={(event) => {
                ...
                <div style={styles.taskPickerActions}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      opacity:
                        normalizedTaskRoot && manualTaskId.trim() ? 1 : 0.52,
                      cursor:
                        normalizedTaskRoot && manualTaskId.trim()
                          ? "pointer"
                          : "default",
                    }}
          ...
          >
            {workstream?.kind === "agent" ? (
              <div style={styles.agentCockpit}>
                <div
                  style={styles.agentMissionPanel}
                  data-testid="agent-cockpit-panel"
                >
                  <div style={styles.agentMissionHeader}>
                    <span style={{ minWidth: 0 }}>
                      <div style={styles.agentMissionLabel}>Task</div>
                      <div
                        style={styles.agentMissionText}
                        title={agentCardTask ?? canvasTaskFallback}
                      >
                        {agentCardTask ?? canvasTaskFallback}
                      </div>
                    </span>
                    <span style={styles.agentStatusPill}>
                      {workstream.phase ?? workstream.status}
                    </span>
                  </div>
                  <div
                    style={styles.agentDecisionRow}
                    aria-label="Agent current activity"
                  >
                    <div
                    ...
                    <div
                      style={styles.agentDecisionCell}
                      title={workstreamActivityMeta(workstream)}
                    >
                      <span style={styles.agentProviderCellLabel}>Signal</span>
                      <span style={styles.agentProviderCellValue}>
                        {workstreamActivityMeta(workstream)}
                      </span>
                    </div>
                  </div>
                  <div
                    style={styles.agentDecisionRow}
                    aria-label="Agent operator guidance"
                  >
                    <div
                      style={styles.agentDecisionCell}
                      title={workstream.lastSummary ?? "No summary yet"}
                    >
                      <span style={styles.agentProviderCellLabel}>Summary</span>
                      <span style={styles.agentProviderCellValue}>
                        {workstream.lastSummary ?? "Starting workstream"}
                      </span>
                    </div>
                    <div
                      style={styles.agentDecisionCell}
                      title={workstream.nextAction ?? "Watch provider response"}
                    >
                      <span style={styles.agentProviderCellLabel}>Next</span>
                      <span style={styles.agentProviderCellValue}>
                        {workstream.nextAction ?? "Watch provider response"}
                      </span>
                    </div>
                  </div>
                  {latestMissionControlInput && (
                    <div
                      style={styles.agentDecisionRow}
                      aria-label="Agent cockpit ask"
                      data-testid="canvas-agent-cockpit-ask"
                    >
                      <div
                        style={styles.agentDecisionCell}
                        title={latestMissionControlInput.text}
                      >
                        <span style={styles.agentProviderCellLabel}>
                          Cockpit ask
                        </span>
                        <span style={styles.agentProviderCellValue}>
                      ...
                      <div
                        style={styles.agentDecisionCell}
                        title={
                          latestMissionControlInput.sentAt
                            ? "Mission-control ask was sent to the provider"
                            : "Mission-control ask is queued for dispatch"
                        }
                      >
                        <span style={styles.agentProviderCellLabel}>
                          Ask state
                  ...
                  >
                    <div
                      style={styles.agentDecisionCell}
                      title={
                        workstream.memory ?? "No agent memory reported yet."
                      }
                    >
                  ...
                  <form
                    style={styles.agentComposer}
                    aria-label="Agent operator composer"
                    onSubmit={onSubmitWorkstreamInput}
                  >
                    <textarea
                      ref={composerRef}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        display: canDraftRecovery
                          ? styles.agentComposerButton.display
                          : "none",
                  ...
                  <details style={styles.agentDetails}>
                    <summary style={styles.agentDetailsSummary}>
                      Details
                    </summary>
                    <div style={styles.agentDetailsBody}>
                      <div
                        style={styles.agentInputStrip}
                        aria-label="Agent input history"
                      >
                        <div
                          style={styles.agentDecisionCell}
                          title={
                            latestInput?.text ?? "No operator input queued yet"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Latest input
                        ...
                        <div
                          ...
                          title={
                            latestInput?.sentAt
                              ? "Prompt has been sent to the provider"
                              : latestInput
                                ? "Prompt is queued for dispatch"
                                : "No prompt yet"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                      ...
                      <div
                        style={styles.agentDecisionRow}
                        aria-label="Agent local context"
                      >
                        <div
                          style={styles.agentDecisionCell}
                          title={workstream.cwd ?? "No launch cwd recorded"}
                        >
                          <span style={styles.agentProviderCellLabel}>Cwd</span>
                          <span style={styles.agentProviderCellValue}>
                            {workstream.cwdLabel ?? "workspace root unknown"}
                          </span>
                        </div>
                        <div
                          style={styles.agentDecisionCell}
                          title={
                            workstream.gitRoot ?? "No git repository detected"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>Git</span>
                          <span style={styles.agentProviderCellValue}>
                            {formatWorkstreamBranch(workstream)}
                          </span>
                        </div>
                      </div>
                      <div
                        style={styles.agentDecisionRow}
                        aria-label="Agent workspace isolation"
                      >
                        <div
                          style={styles.agentDecisionCell}
                          title={
                        ...
                        >
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.worktreePath ?? "unknown"}
                          </span>
                        </div>
                        <div
                          style={styles.agentDecisionCell}
                        ...
                        >
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {formatWorkstreamIsolation(
                              workstream.isolationMode,
                              workstream.isolationStatus,
                            )}
                          </span>
                        </div>
                      </div>
                      <div
                        style={styles.agentDecisionRow}
                        aria-label="Agent worktree cleanup"
                      >
                        <div
                          style={styles.agentDecisionCell}
                          title={
                            workstream.worktreeCleanupNote ??
                            "No cleanup status recorded"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Cleanup
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.worktreeCleanupStatus ?? "unknown"}
                          </span>
                        </div>
                        <div
                          style={styles.agentDecisionCell}
                        ...
                        >
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.worktreeCleanupNote ?? "pending"}
                          </span>
                        </div>
                      </div>
                      <div
                        style={styles.agentDecisionRow}
                        aria-label="Agent output details"
                      >
                        <div
                          style={styles.agentDecisionCell}
                          title={
                            workstream.evidence ?? "No evidence reported yet"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Evidence
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.evidence ?? "pending"}
                          </span>
                        </div>
                        <div
                          style={styles.agentDecisionCell}
                          title={
                            workstream.artifact ?? "No artifact reported yet"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Artifact
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.artifact ?? "pending"}
                          </span>
                        </div>
                      </div>
                      <div
                        style={styles.agentProviderGrid}
                        aria-label="Agent provider control surface"
                      >
                        <div
                          style={styles.agentProviderCell}
                          title={workstream.launchMode ?? "Terminal command"}
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Launch
                          </span>
                          <span style={styles.agentProviderCellValue}>
                            {workstream.launchMode ?? "terminal"}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title={
                            workstream.readinessCheck ??
                            workstream.providerAvailabilityMessage
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.providerAvailable === false
                              ? "unavailable"
                              : (workstream.readiness ?? "unknown")}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title={
                            workstream.authCheck ??
                            "Auth inferred from provider output"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Auth
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.readiness === "auth-required"
                              ? "required"
                              : workstream.readiness === "provider-ready"
                                ? "ready"
                                : "watching"}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title={
                            workstream.structuredStatus
                              ? "Provider-native status"
                              : "Status inferred from terminal output"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Status
                        ...
                        <div
                          style={styles.agentProviderCell}
                          title={
                            workstream.controlProtocol ??
                            workstream.stopBehavior ??
                            "PTY interrupt/kill"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                        ...
                        <div
                          ...
                          title={
                            workstream.stage ??
                            "Provider has not reported a work stage"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Stage
                        ...
                        <div
                          ...
                          title={
                            ...
                            "Provider has not reported confidence"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Confidence
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.confidence ?? "pending"}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title={
                            workstream.risk ?? "Provider has not reported risk"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Risk
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.risk ?? "pending"}
                          </span>
                        </div>
                      </div>
                      <div
                        style={styles.agentRunRecordRow}
                        aria-label="Agent run record"
                      >
                        <div
                          style={styles.agentProviderCell}
                          title={workstream.runId ?? "Run id pending"}
                        >
                          <span style={styles.agentProviderCellLabel}>Run</span>
                          <span style={styles.agentProviderCellValue}>
                            {shortRunId(workstream.runId)} · g
                            {workstream.generation ?? 0}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                        ...
                        >
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.promptCount ?? 0}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title="Prompts sent to the provider"
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Sent
                          </span>
                          <span style={styles.agentProviderCellValue}>
                            {workstream.sentCount ?? 0}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title="Structured provider signals"
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Signals
                          </span>
                          <span style={styles.agentProviderCellValue}>
                            {workstream.signalCount ?? 0}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                        ...
                        >
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {workstream.outcome ?? "Pending"}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title={
                            typeof workstream.exitCode === "number"
                              ? `Provider exited with code ${workstream.exitCode}`
                              : "Provider has not exited"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Exit
                        ...
                        <div
                          ...
                          title={
                            workstream.completedAt
                              ? `Completed ${new Date(workstream.completedAt).toLocaleString()}`
                        ...
                        >
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {runTimestamp(workstream.completedAt)}
                          </span>
                        </div>
                        <div
                          style={styles.agentProviderCell}
                          title={
                            workstream.reviewedAt
                              ? `Reviewed ${new Date(workstream.reviewedAt).toLocaleString()}`
                              : "Run not reviewed yet"
                          }
                        >
                          <span style={styles.agentProviderCellLabel}>
                            Reviewed
                          ...
                          <span style={styles.agentProviderCellValue}>
                            {runTimestamp(workstream.reviewedAt)}
                          </span>
                        </div>
                      </div>
                      <div
                        style={styles.agentTimeline}
                        aria-label="Agent run timeline"
                      >
                        {(workstream.events ?? []).slice(-3).map((event) => (
                          <div
                            key={event.id}
                            style={styles.agentEvent}
                            title={event.detail ?? event.label}
                          >
                            <div style={styles.agentEventTop}>
                              <span style={styles.agentEventKind}>
                                {event.kind}
                              </span>
                              <span style={styles.agentEventTime}>
                                {shortEventTime(event.at)}
                              </span>
                            </div>
                            <div style={styles.agentEventLabel}>
                              {event.label}
                            </div>
                          </div>
                        ))}
                ...
                <div
                  ...
                  style={styles.agentTerminalSlot}
                >
                  {body}
                </div>
              </div>
...
export function MagicCanvas() {
  ...
  const [terminalOverlayRoot, setTerminalOverlayRoot] =
  ...
  const autoFittedCanvasTargetRef = useRef<string | null>(null);
  const [fileIndex, setFileIndex] = useState(0);
  // Map toolbar collapses to a single "Map" chip so it stops covering the canvas.
  // The choice is sticky across relaunches.
  const [toolbarOpen, setToolbarOpen] = useState(() => {
    try {
      return window.localStorage.getItem(MAP_TOOLBAR_OPEN_KEY) !== "0";
    } catch {
      return true;
    }
  });
  ...
  const [terminalPreviews, setTerminalPreviews] = useState<
    Record<string, TerminalPreviewEntry>
  >({});
  // Per-node throttle state for preview updates (leading + trailing).
  const previewThrottleRef = useRef<
    Map<
      string,
      { lastFlush: number; timer: number | null; pending: GridSnapshot | null }
    >
  >(new Map());
  ...
  useEffect(() => {
    if (containerSize.width <= 0 || containerSize.height <= 0 || !nodes.length)
      return;
    const target =
      nodes.find(
        (node) => node.type === "terminal" && node.terminalTabId === activeTabId,
      ) ??
      nodes.find((node) => node.type === "terminal") ??
      selectedCanvasNodes.find((node) => node.type === "terminal") ??
      nodes[0];
    if (!target) return;
    // Fit the initial map target once. Creating or activating another terminal
    // must not steal the operator's current map position; explicit Fit active
    // remains available through the map controls and card actions.
    if (autoFittedCanvasTargetRef.current !== null) return;
    // A persisted viewport belongs to the operator while it still contains the
    // restored target. A stale checkpoint can leave the camera beyond every
    // restored card, though; in that case the restart looked empty even though
    // hydration had rebuilt the terminals correctly. Repair only that case so a
    // deliberate, visible map position remains untouched.
    const viewLeft = -viewport.x / viewport.zoom;
    const viewTop = -viewport.y / viewport.zoom;
    const viewRight = (containerSize.width - viewport.x) / viewport.zoom;
    const viewBottom = (containerSize.height - viewport.y) / viewport.zoom;
  ...
  const selectionToolbarAnchor = useMemo(() => {
    if (selectedCanvasNodes.length < 2) return null;
    ...
    const top = minY * viewport.zoom + viewport.y - 12;
    return {
      left: Math.max(left, SELECTION_TOOLBAR_MIN_LEFT),
      top: Math.max(top, SELECTION_TOOLBAR_MIN_TOP),
    };
  }, [selectedCanvasNodes, viewport.x, viewport.y, viewport.zoom]);

  useEffect(() => {
    try {
      window.localStorage.setItem(
        MAP_TOOLBAR_OPEN_KEY,
        toolbarOpen ? "1" : "0",
      );
    } catch {
      // Storage can be unavailable (private mode); the toolbar still works.
    }
    if (!toolbarOpen) setTidyMenuOpen(false);
  }, [toolbarOpen]);

  useEffect(() => {
    if (!canTidyCanvas) setTidyMenuOpen(false);
  }, [canTidyCanvas]);

  // Dismiss the Tidy popover on outside click or Escape.
  useEffect(() => {
    if (!tidyMenuOpen) return;
    const onPointerDown = (event: MouseEvent) => {
      if (!toolbarRef.current?.contains(event.target as Node))
        setTidyMenuOpen(false);
    };
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setTidyMenuOpen(false);
    };
  ...
  const liveNodeIds = useMemo(() => {
    ...
    const activeTabNodeId = activeTabId
      ? (nodes.find(
          (node) =>
            node.type === "terminal" && node.terminalTabId === activeTabId,
        )?.id ?? null)
      : null;
  ...
  const interruptActiveAgentFleet = useCallback(() => {
    ...
    const store = useWorkspaceStore.getState();
    void Promise.all(
      targets.map(({ tab }) => store.interruptWorkstream(tab.id)),
    );
    setActiveTab(targets[0].tab.id);
  }, [activeAgentWorkstreams, setActiveTab]);
  ...
  const openNodeLabelMenu = useCallback(
  ...
  const createTerminalAt = useCallback(
    async (canvasX: number, canvasY: number) => {
      ...
      if (newTabId) {
        updateCanvasNode(`terminal-map-${newTabId}`, {
  ...
  const flushTerminalPreview = useCallback(
    (nodeId: string, snapshot: GridSnapshot) => {
      setTerminalPreviews((current) => ({
        ...current,
        [nodeId]: {
  ...
  // burst.
  const updateTerminalPreview = useCallback(
    (nodeId: string, snapshot: GridSnapshot) => {
  ...
  const startCanvasPan = useCallback(
    (event: React.MouseEvent, options?: { deselectOnClick?: boolean }) => {
      event.preventDefault();
      event.stopPropagation();
      let moved = false;
      panRef.current = {
      ...
      function applyPanToDom() {
        ...
        if (!pan) return;
        if (stageRef.current) {
          stageRef.current.style.transform = `translate(${pan.nextX}px, ${pan.nextY}px) scale(${pan.zoom})`;
        }
        if (shellRef.current) {
      ...
      function onMouseMove(moveEvent: MouseEvent) {
        ...
        if (!pan) return;
        if (
          Math.abs(moveEvent.clientX - pan.x) > 3 ||
          Math.abs(moveEvent.clientY - pan.y) > 3
        ) {
          moved = true;
        }
  ...
  const onCanvasMouseDown = useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (event.button === 0 && event.shiftKey && rect) {
        ...
        const canvasX =
          (event.clientX - canvasRect.left - viewport.x) / viewport.zoom;
        const canvasY =
          (event.clientY - canvasRect.top - viewport.y) / viewport.zoom;
        ...
        function onSelectUp(upEvent: MouseEvent) {
          ...
          if (!start) return;
          const endCanvasX =
            (upEvent.clientX - canvasRect.left - viewport.x) / viewport.zoom;
          const endCanvasY =
            (upEvent.clientY - canvasRect.top - viewport.y) / viewport.zoom;
          ...
          const selectedIds = useWorkspaceStore
            .getState()
            .canvasState.nodes.filter((node) => node.type === "terminal")
            .filter((node) =>
              rectsIntersect(selectionRect, {
                minX: node.x,
                minY: node.y,
  ...
  const addFile = useCallback(() => {
    ...
    addCanvasNode({
      ...
      content:
        file?.path ??
        "Open a file from the explorer, then add another file node.",
    });
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          ...styles.toolbar,
          ...(toolbarOpen ? null : styles.toolbarCollapsed),
        }}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <button
          type="button"
          className="magic-canvas-chip"
        ...
        {toolbarOpen && (
          <>
            ...
            <button
              ...
              title="Add a drawing board to the map"
              aria-label="Add drawing board"
              onClick={addBoard}
            >
            ...
            <button
              className="magic-canvas-button"
              style={styles.toolButton}
              title="Add a terminal to the map"
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                onClick={() => setTidyMenuOpen((open) => !open)}
              >
              ...
              {tidyMenuOpen && (
                <div
                  className="magic-canvas-glass"
                  style={styles.tidyMenu}
                  role="menu"
                >
                  <button
                    className="magic-canvas-button"
                  ...
                  >
                    ...
                    Group every project into lanes
                  </button>
                </div>
              )}
            </div>
      ...
      {selectionToolbarAnchor && (
        <div
          ...
          style={{
            ...
            left: selectionToolbarAnchor.left,
            top: selectionToolbarAnchor.top,
          }}
          onMouseDown={(event) => event.stopPropagation()}
          role="toolbar"
        ...
        >
          <span style={styles.selectionCount}>
            {selectedCanvasNodes.length} selected
          </span>
          <span style={styles.toolbarDivider} aria-hidden="true" />
          <button
            className="magic-canvas-button"
            ...
            aria-label="Line their left edges up"
          ...
          <button
            ...
            aria-label="Line their top edges up"
          ...
          <button
            className="magic-canvas-button"
            style={styles.iconButton}
            title="Centre them side to side"
            aria-label="Centre them side to side"
          ...
          <button
            className="magic-canvas-button"
            style={styles.iconButton}
            title="Centre them top to bottom"
            aria-label="Centre them top to bottom"
            onClick={() => alignCanvasNodes(selectedCanvasNodeIds, "center-y")}
          >
            <AlignVerticalJustifyCenter size={14} strokeWidth={1.8} />
          </button>
          {canDistributeCanvasNodes && (
            <>
              <span style={styles.toolbarDivider} aria-hidden="true" />
              <button
                className="magic-canvas-button"
                style={styles.iconButton}
                title="Even out the gaps left to right"
                aria-label="Even out the gaps left to right"
                onClick={() =>
                  distributeCanvasNodes(selectedCanvasNodeIds, "horizontal")
                }
              >
              ...
              <button
                className="magic-canvas-button"
                style={styles.iconButton}
                title="Even out the gaps top to bottom"
                aria-label="Even out the gaps top to bottom"
                onClick={() =>
                  distributeCanvasNodes(selectedCanvasNodeIds, "vertical")
                }
              >
      ...
      {agentLane.total > 0 && (
        ...
        >
          ...
          <div style={styles.agentLaneStats}>
            <span
              style={styles.agentLaneChip}
              data-testid="canvas-agent-lane-total"
            >
          ...
          {agentLane.primaryAttention && (
            <button
              ...
              onClick={() => {
                ...
                if (!attention) return;
                const node = canvasState.nodes.find(
                  (candidate) => candidate.terminalTabId === attention.tabId,
                );
            ...
            >
              ...
              {agentLane.attentionItems.length > 3 && (
                <div
                  ...
                  data-testid="canvas-agent-attention-overflow"
                >
          ...
          <div style={styles.agentLaneList} aria-label="Agent workspace groups">
            ...
            {agentLane.workspaceGroups.length > 3 && (
              <div
                style={styles.agentLaneOverflow}
                data-testid="canvas-agent-workspace-group-overflow"
              >
          ...
          {agentLane.inputItems.length > 0 && (
            ...
            >
              ...
              {agentLane.inputItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.inputItems.length - 3} more prompts
                  </span>
                  <span>
          ...
          {agentLane.outputItems.length > 0 && (
            ...
            >
              ...
              {agentLane.outputItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.outputItems.length - 3} more output
                  </span>
                  <span>
          ...
          {agentLane.nextItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent next actions">
              ...
              {agentLane.nextItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.nextItems.length - 3} more next
                  </span>
                  <span>
          ...
          {agentLane.staleItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent stale queue">
              ...
              {agentLane.staleItems.length > 3 && (
                <div
                  style={styles.agentLaneOverflow}
                  data-testid="canvas-agent-stale-overflow"
                >
          ...
          {agentLane.riskItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent risk queue">
              ...
              {agentLane.riskItems.length > 3 && (
                <div
                  style={styles.agentLaneOverflow}
                  data-testid="canvas-agent-risk-overflow"
                >
          ...
          {agentLane.authItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent auth queue">
              ...
              {agentLane.authItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.authItems.length - 3} more auth
                  </span>
                  <span>
          ...
          {agentLane.recoveryItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent recovery queue">
              ...
              {agentLane.recoveryItems.length > 3 && (
                <div
                  style={styles.agentLaneOverflow}
                  data-testid="canvas-agent-recovery-overflow"
                >
          ...
          {agentLane.proofItems.length > 0 && (
            ...
            >
              ...
              {agentLane.proofItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.proofItems.length - 3} more proof
                  </span>
                  <span>
          ...
          {agentLane.reviewItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent review queue">
              ...
              {agentLane.reviewItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.reviewItems.length - 3} more review
                  </span>
                  <span>
          ...
          {agentLane.memoryItems.length > 0 && (
            <div style={styles.agentLaneList} aria-label="Agent lane memory">
              ...
              {agentLane.memoryItems.length > 3 && (
                ...
                >
                  <span style={{ color: "var(--text-primary)" }}>
                    +{agentLane.memoryItems.length - 3} more memory
                  </span>
                  <span>
          ...
          <div style={styles.agentLaneList}>
            {agentLane.workstreams.slice(0, 3).map(({ tab, workstream }) => {
              ...
              return (
                ...
                >
                  ...
                  <span>
                    {workstreamLabel(workstream.provider)} ·{" "}
                    {workstream.phase ?? workstream.status} ·{" "}
                    {workstreamActivityText(workstream)} ·{" "}
            ...
            {agentLane.workstreams.length > 3 && (
              ...
              >
                <span style={{ color: "var(--text-primary)" }}>
                  +{agentLane.workstreams.length - 3} more agents
                </span>
                <span>
                  {workstreamLabel(
                  ...
                  ·{" "}
                  {agentLane.workstreams[3].workstream.phase ??
                    agentLane.workstreams[3].workstream.status}{" "}
                  ·{" "}
                  {workstreamActivityText(agentLane.workstreams[3].workstream)}{" "}
                  ·{" "}
                  {formatWorkstreamOpsContext(
      ...
      {canvasState.nodes.length === 0 && (
        <div style={styles.empty}>
          Map is empty. Add a note, shell, or file node.
        </div>
      )}
              ...
              },
              {
                ...
                run: () =>
                  addCanvasNode({
                    ...
                    height: 160,
                    content:
                      "Capture the command, blocker, or next action for this run.",
                  }),
              },
              {
