...
This increment delivers **BiDi reordering only** (TC-018 Stages 1–2). After it,
...
**unchanged**; React keeps receiving a plain visual-coordinate grid.
...
## Explicitly deferred (Stage 3 fast-follow)

Nikud/combining-mark stacking, `rustybuzz` shaping, the 16-byte cell payload,
and canvas glyph-ID re-keying (TC-018 Stages 3–5).
