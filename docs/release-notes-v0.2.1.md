# TermFleet v0.2.1 — early preview (Linux only)
...
- **Connect agents.** v0.2.0 shipped without a way to connect Claude Code,
  Codex, or OpenCode, so agent panes in a downloaded build had no Task line or
  Running / Waiting / Idle badge. The status hooks now ship inside the app: the
  status bar offers **Connect agents** when an agent is set up for your user,
  copies the hooks to `~/.local/share/termfleet/agent-hooks/`, registers them,
  and backs up every settings file it touches. Needs Node.js 20+.
- The release is verified by launching the published AppImage and `.deb` with an
  empty profile, connecting agents, and emitting a real Claude hook event.
...
- A brand-new install labels its first terminal "reconnected" in the status bar.
