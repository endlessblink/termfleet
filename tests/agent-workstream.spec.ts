  ...
  const queued = await page.evaluate(
    ({ text, mission }) => {
        ...
        window as typeof window & {
          __termfleetWorkspaceStore?: {
            getState: () => {
              tabs: Array<{
                id: string;
                workstream?: { kind?: string; mission?: string };
              }>;
      ...
      const candidates = state.tabs.filter(
        (tab) => tab.workstream?.kind === "agent",
      );
      const tab = mission
        ? candidates.find(
            (candidate) => candidate.workstream?.mission === mission,
          )
        : candidates.at(-1);
      if (!tab)
        throw new Error(
          `Agent workstream not found${mission ? `: ${mission}` : ""}`,
        );
...
test("restored agent lanes keep one visible map node with restore state after reloads", async ({
  ...
  await expect(page.getByTestId("canvas-agent-working-on")).toContainText(
    seeded.mission,
  );
  await expect(page.getByTestId("canvas-agent-status-now")).toContainText(
    "Resuming Codex session",
  );
  await expect(page.getByTestId("canvas-agent-status-chips")).toContainText(
    "restore · resuming",
  );
  await expect(page.getByTestId("canvas-agent-node-workspace")).toContainText(
    "termfleet",
  );
  await expect(page.getByTestId("canvas-agent-lane-total")).toHaveText(
    "1 agents",
  );
  ...
  const hydrated = await page.evaluate((seed) => {
      ...
      window as typeof window & {
        __termfleetWorkspaceStore?: {
          getState: () => {
            ...
            canvasState: {
              nodes: Array<{ id: string; terminalTabId?: string }>;
    ...
    return {
      ...
      agentCount: state.tabs.filter((tab) => tab.workstream?.kind === "agent")
        .length,
      nodeCount: state.canvasState.nodes.filter(
        (node) => node.id === seed.nodeId && node.terminalTabId === seed.tabId,
      ).length,
      selectedNodeId: state.canvasState.selectedNodeId,
      providerSessionId: state.tabs.find((tab) => tab.id === seed.tabId)
        ?.workstream?.providerSessionId,
      restoreStatus: state.tabs.find((tab) => tab.id === seed.tabId)?.workstream
        ?.restoreStatus,
    };
  }, seeded);
  ...
  await expect(page.getByTestId("split-agent-working-on")).toContainText(
    seeded.mission,
  );
  await expect(
    page.getByText("codex · working · restore · resuming"),
  ).toBeVisible();
});

test("agent terminal task sidebar does not rewrite from extracted live summaries", async ({
  page,
}) => {
  await resetWorkspace(page);
      ...
      window as typeof window & {
        __termfleetWorkspaceStore?: {
          getState: () => {
            ...
            updateTab: (
              id: string,
              updates: { workstream?: Record<string, unknown> },
            ) => void;
          };
        };
      }
      ...
      workstream: {
        ...
        cockpitObjects: [
          ...
          object(
            "evidence",
            "evidence-1",
            "npm test -- checkout-flow.spec passed",
            "Evidence: npm test -- checkout-flow.spec passed",
            2,
          ),
          object(
            "next-action",
            "next-1",
            "Rerun checkout-flow.spec with retry trace",
  ...
  await runWorkspaceCommand(page, "show terminal");
  await expect(page.getByTestId("split-agent-task-sidebar")).toContainText(
    "Tasks",
  );
  await expect(page.getByTestId("split-agent-task-row")).toHaveCount(1);
  await expect(page.getByTestId("split-agent-task-row")).toContainText(
    "Persist retry evidence on the checkout report",
  );
  await expect(page.getByTestId("split-agent-task-state")).toContainText(
    "Working",
  );
  await expect(page.getByTestId("split-agent-task-next")).toContainText(
    "Next: Rerun checkout-flow.spec with retry trace",
  );
});

async function ageAgentWorkstream(
  page: import("@playwright/test").Page,
  mission: string,
  idleMinutes: number,
  activity: string,
    ...
    ({ mission, idleMinutes, activity }) => {
      ...
      const agent = state.tabs?.find(
        (tab) =>
          ...
          tab.workstream?.mission === mission,
      );
      if (!agent?.workstream)
        throw new Error(`Agent workstream not found: ${mission}`);
  ...
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "1 stale",
  );
}

async function seedAgentTerminalOutputs(
  page: import("@playwright/test").Page,
  missions: string[],
) {
  await page.evaluate((missions) => {
    ...
    for (const [index, mission] of missions.entries()) {
      const agent = state.tabs?.find(
        (tab: { workstream?: { kind?: string; mission?: string } }) =>
          tab.workstream?.kind === "agent" &&
          tab.workstream?.mission === mission,
      );
      if (!agent?.workstream)
...
async function seedAgentMemories(
  page: import("@playwright/test").Page,
  missions: string[],
) {
  await page.evaluate((missions) => {
...
async function seedAgentEvidence(
  page: import("@playwright/test").Page,
  missions: string[],
) {
  await page.evaluate((missions) => {
...
async function seedAgentAuthRequired(
  page: import("@playwright/test").Page,
  missions: string[],
) {
  await page.evaluate((missions) => {
...
async function seedAgentRecovery(
  page: import("@playwright/test").Page,
  missions: string[],
) {
  await page.evaluate((missions) => {
...
async function seedTwoProviderScanRows(page: import("@playwright/test").Page) {
  await page.evaluate(() => {
    ...
    const state = store.getState();
    const now = Date.now();
    const audit = state.tabs?.find(
      (tab) =>
        tab.workstream?.kind === "agent" &&
        tab.workstream?.mission === "Audit deployment scripts",
    );
    const release = state.tabs?.find(
      (tab) =>
        tab.workstream?.kind === "agent" &&
        tab.workstream?.mission === "Prepare release notes",
    );
    if (!audit?.workstream)
      throw new Error("Audit deployment scripts workstream not found");
    if (!release?.workstream)
      throw new Error("Prepare release notes workstream not found");
...
async function seedAgentWorkspaceGroups(
  page: import("@playwright/test").Page,
  missions: string[],
) {
  await page.evaluate((missions) => {
...
async function seedAgentCleanupOwnership(
  page: import("@playwright/test").Page,
) {
  await page.evaluate(() => {
    const store = (
    ...
    const updates = [
      {
        ...
        worktreeCleanupNote:
          "Dedicated worktree can be removed after closeout.",
    ...
    for (const [index, update] of updates.entries()) {
      const agent = state.tabs?.find(
        (tab) =>
          tab.workstream?.kind === "agent" &&
          tab.workstream?.mission === update.mission,
      );
      if (!agent?.workstream)
        throw new Error(`Agent workstream not found: ${update.mission}`);
      store.getState().updateTab(agent.id, {
        workstream: {
          ...agent.workstream,
          ...update,
...
test("command palette creates a supervised Codex agent on the map", async ({
  page,
  context,
}) => {
  ...
  await expect(page.getByTestId("canvas-agent-lane-headline")).toContainText(
    "Running",
  );
  await expect(page.getByTestId("canvas-agent-lane-headline")).toContainText(
    "1 active agent",
  );
  await expect(page.getByTestId("canvas-agent-lane-health")).toContainText(
    "Running",
  );
  await expect(page.getByTestId("canvas-agent-lane-health")).toContainText(
    "1 agents",
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-provider-breakdown"),
  ).toContainText("Codex: 1");
  await expect(
    page.getByTestId("canvas-agent-lane-isolation-breakdown"),
  ).toContainText("shared workspace: 1");
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "1 prompts",
  );
  await expect(page.getByTestId("canvas-agent-input-item")).toContainText(
    "Copy prompt",
  );
  await expect(page.getByTestId("canvas-agent-input-item")).toContainText(
    PRIMARY_MISSION,
  ...
  await expect(page.getByTestId("map-agent-lane-headline")).toContainText(
    "Running",
  );
  await expect(page.getByTestId("map-agent-lane-health")).toContainText(
    "Running",
  );
  await expect(page.getByTestId("map-agent-lane-health")).toContainText(
    "1 agents",
  ...
  await expect(
    page.getByTestId("map-agent-lane-provider-breakdown"),
  ).toContainText("Codex: 1");
  await expect(
    page.getByTestId("map-agent-lane-isolation-breakdown"),
  ).toContainText("shared workspace: 1");
  await expect(page.getByTestId("map-agent-lane-summary")).toContainText(
    "1 prompts",
  );
  await expect(page.getByTestId("map-agent-input-item")).toContainText(
    PRIMARY_MISSION,
  );
  await page.getByTestId("canvas-agent-input-item").click();
  const copiedInitialPrompt = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedInitialPrompt.toBe(
    `${PRIMARY_MISSION}: sent - ${PRIMARY_MISSION}`,
  );
  await expect(page.getByTestId("agent-cockpit-panel")).toBeVisible();
  await expect(
    page.getByTestId("agent-cockpit-panel").getByText("Task", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText(PRIMARY_MISSION).first()).toBeVisible();
  await expect(page.getByTestId("canvas-agent-working-on")).toContainText(
    PRIMARY_MISSION,
  );
  await expect(page.getByTestId("canvas-agent-task-sidebar")).toContainText(
    "Tasks",
  );
  await expect(page.getByTestId("canvas-agent-task-row")).toContainText(
    PRIMARY_MISSION,
  );
  await expect(page.getByTestId("canvas-agent-task-state")).toContainText(
    "Working",
  );
  await expect(page.getByTestId("canvas-agent-task-next")).toContainText(
    "Next: Watch provider response",
  );
  await expect(page.getByTestId("canvas-agent-status-path")).toContainText(
    "Path",
  );
  await expect(page.getByTestId("canvas-agent-status-now")).toContainText(
  ...
  await expect(page.getByTestId("split-agent-status-path")).toContainText(
    "workspace root unknown",
  );
  await expect(page.getByTestId("split-agent-pane-now")).toContainText(
    "Watch provider response",
  );
  await expect(page.getByTestId("split-agent-task-sidebar")).toContainText(
    "Tasks",
  ...
  await expect(page.getByLabel("Agent current activity")).toContainText(
    "command is not available in browser preview",
  );
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Watch provider response"),
  ).toBeVisible();
  await expect(page.getByText("Details")).toBeVisible();
  await page.getByText("Details").click();
  await expect(
    page
      .getByLabel("Agent provider control surface")
      .getByText("interactive CLI", { exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByLabel("Agent local context")
      .getByText("workspace root unknown", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent local context")
      .getByText("branch unknown · state unknown", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent workspace isolation")
      .getByText("shared workspace", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent worktree cleanup")
      .getByText("not-needed", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Prompts"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Outcome"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Run", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Done"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Reviewed"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Provider is running"),
  ).toBeVisible();
  ...
  await expect
      ...
      page.evaluate(() => {
        ...
        return {
          ...
          providerAvailabilityMessage:
            agent?.workstream?.providerAvailabilityMessage,
          ...
          events: agent?.workstream?.events?.map(
            (event: {
              kind?: string;
              label?: string;
              detail?: string;
          ...
          initialInput: agent?.workstream?.inputQueue?.find(
            (input: { text?: string }) =>
    ...
    .toMatchObject({
      ...
      providerAvailabilityMessage:
        "Browser preview simulates provider startup; desktop checks PATH before launch.",
      startupCommand: "codex",
      ...
      readinessCheck:
        "PATH check only; auth/session readiness is confirmed by CLI output.",
      authCheck:
        "CLI output scan for login, API key, OAuth, or sign-in prompts.",
      readiness: "path-checked",
      stopBehavior:
        "PTY interrupt/kill until provider-native cancel is available.",
      controlProtocol: "TermFleet prompt queue plus PTY Ctrl-C/kill fallback.",
      structuredStatus: false,
      currentActivity: expect.stringMatching(
        /^(Prompt sent to provider|Provider is running|Investigate flaky checkout flow: command is not available in browser preview\. Use the Tauri app for real shell commands\.|codex: command is not available in browser preview\. Use the Tauri app for real shell commands\.)$/,
      ),
      ...
      lastSummary: expect.stringMatching(
        /^(Prompt sent to provider|Provider is running)$/,
      ),
      ...
      events: expect.arrayContaining([
        ...
        expect.objectContaining({
          kind: "status",
          label: "Status changed to running",
          status: "running",
        }),
      ]),
      initialInput: {
  ...
  await expect(
    page.getByRole("form", { name: "Agent operator composer" }),
  ).toBeVisible();
  await expect(page.getByLabel("Agent current activity")).toContainText(
    /Prompt sent to provider|Provider is running|command is not available in browser preview/,
  );
  await expect(page.getByLabel("Agent current activity")).toContainText(
    /thinking · operator|running · terminal|blocked · terminal/,
  );
  await expect(
    page.getByTestId("canvas-agent-lane-provider-breakdown"),
  ).toContainText("Codex: 1");
  await expect(
    page.getByTestId("canvas-agent-lane-readiness-breakdown"),
  ).toContainText("Path checked: 1");
  await expect(
    page.getByTestId("map-agent-lane-provider-breakdown"),
  ).toContainText("Codex: 1");
  await expect(
    page.getByTestId("map-agent-lane-readiness-breakdown"),
  ).toContainText("Path checked: 1");
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "0 memories",
  );
  ...
  await expect
    .poll(async () =>
      page.evaluate(() => {
        ...
        const launchInput = agent?.workstream?.inputQueue?.find(
          (item: { text?: string }) =>
            item.text === "Investigate flaky checkout flow",
        );
        const input = agent?.workstream?.inputQueue?.find(
          (item: { text?: string }) => item.text === "echo waiting for input",
        );
        const ptys =
          (
            window as typeof window & {
              __terminalWorkspaceBrowserPtys?: Record<
                string,
    ...
    .toEqual({
      ...
      phase: "needs-input",
      currentActivity: expect.stringMatching(
        /^(Provider is waiting for operator input|waiting for input)$/,
      ),
      ...
      events: expect.arrayContaining([
        ...
        expect.objectContaining({
          kind: "status",
          label: "Status changed to waiting",
          status: "waiting",
        }),
      ]),
  ...
  await expect(
    page.getByTestId("agent-cockpit-panel").getByText("Follow-up queued"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Provider is waiting for operator input"),
  ).toBeVisible();
  await expect(page.getByLabel("Agent current activity")).toContainText(
    /Provider is waiting for operator input|waiting for input/,
  );
  ...
  await expect(page.getByTestId("canvas-agent-output-item")).toContainText(
    "Copy output",
  );
  await expect(page.getByTestId("canvas-agent-output-item")).toContainText(
    "waiting for input",
  );
  await expect(page.getByTestId("map-agent-output-item")).toContainText(
    "waiting for input",
  );
  await page.getByTestId("canvas-agent-output-item").click();
  const copiedOutput = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedOutput.toContain(`${PRIMARY_MISSION}:`);
  await copiedOutput.toContain("waiting for input");
  await expect(page.getByTestId("canvas-agent-next-item")).toContainText(
    "Copy next",
  );
  await expect(page.getByTestId("canvas-agent-next-item")).toContainText(
    "Send a follow-up prompt",
  );
  await expect(page.getByTestId("map-agent-next-item")).toContainText(
    "Send a follow-up prompt",
  );
  await page.getByTestId("canvas-agent-next-item").click();
  const copiedNextAction = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedNextAction.toBe(
    `${PRIMARY_MISSION}: next - Send a follow-up prompt`,
  );
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Send a follow-up prompt"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent input history").getByText("Latest input"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent input history").getByText("echo waiting for input"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent input history").getByText("sent", { exact: true }),
  ).toBeVisible();
  await runWorkspaceCommand(page, "show terminal");
  await expect(page.getByTestId("split-agent-pane-output")).toContainText(
    "Output:",
  );
  await expect(page.getByTestId("split-agent-pane-output")).toContainText(
    "waiting for input",
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          sentCount: agent?.workstream?.sentCount,
          outcome: agent?.workstream?.outcome,
          hasAuthEvent: agent?.workstream?.events?.some(
            (event: { kind?: string; label?: string }) =>
              event.kind === "provider" &&
              event.label === "Provider auth required",
          ),
        };
      }),
    )
  ...
  await page
    .getByTestId("agent-cockpit-panel")
    .locator("details")
    .first()
    .evaluate((node) => {
  ...
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Provider requires authentication"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent current activity")
      .getByText("Provider requires authentication"),
  ).toBeVisible();
  await expect(page.getByTestId("canvas-agent-lane-attention")).toContainText(
    "Auth required",
  );
  await expect(page.getByTestId("canvas-agent-lane-attention")).toContainText(
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-attention-breakdown"),
  ).toContainText("Auth required: 1");
  await expect(
    page.getByTestId("canvas-agent-lane-readiness-breakdown"),
  ).toContainText("Auth required: 1");
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
  ...
  await expect(page.getByTestId("canvas-agent-attention-item")).toContainText(
    "Auth required",
  );
  await expect(page.getByTestId("canvas-agent-attention-item")).toContainText(
    "Authenticate the CLI",
  );
  await expect(page.getByTestId("map-agent-lane-attention")).toContainText(
    "Auth required",
  );
  await expect(
    page.getByTestId("map-agent-lane-attention-breakdown"),
  ).toContainText("Auth required: 1");
  await expect(
    page.getByTestId("map-agent-lane-readiness-breakdown"),
  ).toContainText("Auth required: 1");
  await expect(page.getByTestId("map-agent-attention-item")).toContainText(
    "Auth required",
  );
  await expect(page.getByTestId("canvas-agent-auth-item")).toContainText(
    "Copy auth",
  );
  await expect(page.getByTestId("canvas-agent-auth-item")).toContainText(
  ...
  await expect(page.getByTestId("map-agent-auth-item")).toContainText(
    "Provider requires authentication",
  );
  await page.getByTestId("canvas-agent-auth-item").click();
  const copiedAuthItem = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedAuthItem.toContain(
    `${PRIMARY_MISSION}: Provider requires authentication`,
  );
  await copiedAuthItem.toContain(
    "next=Authenticate the CLI, then restart or send a recovery prompt",
  );
  await copiedAuthItem.toContain(
    "readiness=PATH check only; auth/session readiness is confirmed by CLI output.",
  );
  await copiedAuthItem.toContain(
    "auth=CLI output scan for login, API key, OAuth, or sign-in prompts.",
  );
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedAuthBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedAuthBrief.toContain("Readiness mix: Auth required: 1");
  await copiedAuthBrief.toContain("Auth queue:");
  await copiedAuthBrief.toContain(
    `${PRIMARY_MISSION}: Provider requires authentication`,
  );
  await copiedAuthBrief.toContain(
    "Next: Authenticate the CLI, then restart or send a recovery prompt",
  ...
  const agentTabId = await page.evaluate(() => {
    ...
    const state = raw ? JSON.parse(raw) : null;
    const agent = state?.tabs?.find(
      (tab: { id?: string; workstream?: { kind?: string } }) =>
        tab.workstream?.kind === "agent",
    );
    return agent?.id;
  });
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          exitCode: agent?.workstream?.exitCode,
          outcome: agent?.workstream?.outcome,
          hasFailureSignal: agent?.workstream?.events?.some(
            (event: { kind?: string; label?: string }) =>
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-risk-breakdown"),
  ).toContainText("low confidence: 1");
  await expect(page.getByTestId("canvas-agent-evidence-item")).toContainText(
    "stderr: provider exited 2",
  );
  await expect(page.getByTestId("canvas-agent-evidence-item")).toContainText(
    "logs/provider-crash.txt",
  );
  await expect(page.getByTestId("canvas-agent-risk-item")).toContainText(
    "confidence=low",
  );
  await expect(page.getByTestId("canvas-agent-risk-item")).toContainText(
    "risk=provider crashed before saving state",
  );
  await expect(page.getByTestId("map-agent-lane-risk-breakdown")).toContainText(
    "low confidence: 1",
  );
  await expect(page.getByTestId("map-agent-risk-item")).toContainText(
    "provider crashed before saving state",
  );
  await expect(page.getByTestId("canvas-agent-recovery-item")).toContainText(
    PRIMARY_MISSION,
  );
  await expect(page.getByTestId("canvas-agent-recovery-item")).toContainText(
    "Provider crashed",
  ...
  await expect(page.getByTestId("map-agent-recovery-item")).toContainText(
    "Recover Codex agent",
  );
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedRecoveryBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedRecoveryBrief.toContain("Risk queue:");
  await copiedRecoveryBrief.toContain("Risk mix: low confidence: 1");
  await copiedRecoveryBrief.toContain(
    "confidence=low · risk=provider crashed before saving state",
  );
  await copiedRecoveryBrief.toContain("Attention mix: Blocked: 1");
  await copiedRecoveryBrief.toContain("Recovery queue:");
  await copiedRecoveryBrief.toContain(
    "Prompt: Recover Codex agent: inspect the failure output, summarize the root cause, and propose the next command.",
  );
  await expect(
    page
      .getByLabel("Agent current activity")
      .getByText("Inspecting provider crash", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent current activity")
      .getByText("blocked · structured", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent operator guidance").getByText("Provider crashed"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent output details")
      .getByText("stderr: provider exited 2"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent output details")
      .getByText("logs/provider-crash.txt"),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent provider control surface")
      .getByText("failure analysis", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent provider control surface")
  ...
  await expect(
    page
      .getByLabel("Agent provider control surface")
      .getByText("provider crashed before saving state", { exact: true }),
  ).toBeVisible();
  ...
  await expect
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          latestInputSent: Boolean(
            agent?.workstream?.inputQueue?.at(-1)?.sentAt,
          ),
          ...
          hasMissionControlQueuedEvent: agent?.workstream?.events?.some(
            (event: { label?: string }) =>
              event.label === "Mission control queued Mitigate risk",
          ),
          hasMissionControlSentEvent: agent?.workstream?.events?.some(
            (event: { label?: string }) =>
    ...
    .toEqual({
      ...
      currentActivity: expect.stringMatching(
        /^(Resolve risk for Codex agent|Provider requires authentication)/,
      ),
      lastEvent: expect.stringMatching(
        /^(Mission control: Mitigate risk sent|Status changed to waiting)$/,
      ),
  ...
  await expect(page.getByTestId("canvas-agent-cockpit-ask")).toContainText(
    "Cockpit ask",
  );
  await expect(page.getByTestId("canvas-agent-cockpit-ask")).toContainText(
    "Resolve risk for Codex agent",
  );
  await expect(page.getByTestId("canvas-agent-cockpit-ask")).toContainText(
    "Mitigate risk · sent",
  );
  await runWorkspaceCommand(page, "show terminal");
  await expect(page.getByTestId("split-agent-pane-ask")).toContainText(
    "Ask · Mitigate risk · sent",
  );
  await expect(page.getByTestId("split-agent-pane-ask")).toContainText(
    "Resolve risk for Codex agent",
  );
  ...
  await expect
    ...
    .toEqual({
      phase: expect.stringMatching(/^(launching|needs-input)$/),
      latestInput:
        "Recover Codex agent: inspect the failure output, summarize the root cause, and propose the next command.",
      latestInputSent: true,
      latestInputSource: "mission-control",
      latestInputLabel: "Recover",
      currentActivity: expect.stringMatching(
        /^(Recover Codex agent|Provider requires authentication)/,
      ),
      lastEvent: expect.stringMatching(
        /^(Mission control: Recover sent|Status changed to waiting|Provider auth required)$/,
      ),
      hasMissionControlQueuedEvent: true,
      hasMissionControlSentEvent: true,
      promptCount: 6,
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          sentCount: agent?.workstream?.sentCount,
          outcome: agent?.workstream?.outcome,
          hasReadyEvent: agent?.workstream?.events?.some(
            (event: { kind?: string; label?: string }) =>
              event.kind === "provider" &&
              event.label === "Provider session ready",
          ),
        };
      }),
    )
  ...
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Provider session is ready"),
  ).toBeVisible();
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          reviewedAt: agent?.workstream?.reviewedAt,
          lastEvent: agent?.workstream?.events?.at(-1)?.label,
          outputHasInterrupt: Object.values(
            (
              window as typeof window & {
                __terminalWorkspaceBrowserPtys?: Record<
                  string,
          ...
          ).some((session) => session.output.includes("^C")),
        };
      }),
    )
    .toEqual({
      ...
      currentActivity: expect.stringMatching(
        /^(Cancellation requested|Provider session is ready)$/,
      ),
      ...
      nextAction: expect.stringMatching(
        /^(Wait for provider acknowledgement or hard-stop|Send a task or watch provider response)$/,
      ),
      promptCount: 7,
      sentCount: 7,
      controlCount: 1,
  ...
  await expect(page.getByLabel("Agent operator guidance")).toContainText(
    /Cancellation requested|Provider session is ready/,
  );
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          controlCount: agent?.workstream?.controlCount,
          outcome: agent?.workstream?.outcome,
          hasInterruptedEvent: agent?.workstream?.events?.some(
            (event: { kind?: string; label?: string }) =>
              event.kind === "provider" &&
              event.label === "Provider interrupted",
          ),
        };
      }),
    )
  ...
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Provider acknowledged cancellation"),
  ).toBeVisible();
  ...
  await expect
    ...
    .toEqual({
      ...
      phase: "active",
      currentActivity: expect.stringMatching(
        /^(Provider is running|codex: command is not available in browser preview\. Use the Tauri app for real shell commands\.)$/,
      ),
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          reviewedAt: agent?.workstream?.reviewedAt,
          lastEvent: agent?.workstream?.events?.at(-1)?.label,
          hasStructuredSignal: agent?.workstream?.events?.some(
            (event: { kind?: string; label?: string }) =>
              event.kind === "signal" &&
              event.label === "Structured completion",
          ),
        };
      }),
    )
  ...
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Structured task completed", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Review structured result", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent current activity")
      .getByText("Reviewing checkout report", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent current activity")
      .getByText("complete · structured", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent memory")
      .getByText(
        "Checkout flake isolated to retry timing; preserve auth fixture logs.",
        { exact: true },
      ),
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-readiness-breakdown"),
  ).toContainText("Provider ready: 1");
  await expect(
    page.getByTestId("map-agent-lane-readiness-breakdown"),
  ).toContainText("Provider ready: 1");
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
  ...
  await expect(page.getByTestId("canvas-agent-lane-closeout-plan")).toHaveText(
    "Review 1 ready",
  );
  await expect(page.getByTestId("map-agent-lane-closeout-plan")).toHaveText(
    "Review 1 ready",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-review-ready"),
  ).toBeEnabled();
  ...
  await expect(page.getByTestId("canvas-agent-review-item")).toContainText(
    "Ready with proof",
  );
  await expect(page.getByTestId("canvas-agent-review-item")).toContainText(
    "Memory ready",
  ...
  await expect(page.getByTestId("canvas-agent-review-item")).toContainText(
    "reports/checkout-flow.md",
  );
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Review" }),
  ).toContainText("Ready with proof");
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Review" }),
  ).toContainText("Memory ready");
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Review" }),
  ).toContainText("Structured task completed");
  await expect(page.getByTestId("map-agent-review-item")).toContainText(
    "Ready with proof",
  );
  await expect(page.getByTestId("map-agent-review-item")).toContainText(
    "Memory ready",
  ...
  await expect(page.getByTestId("canvas-agent-evidence-item")).toContainText(
    "tests: checkout-flow.spec passed",
  );
  await expect(page.getByTestId("canvas-agent-evidence-item")).toContainText(
    "reports/checkout-flow.md",
  );
  await expect(page.getByTestId("map-agent-evidence-item")).toContainText(
    "tests: checkout-flow.spec passed",
  );
  ...
  await expect(structuredCompletionEvent).toContainText(
    "Structured completion",
  );
  await expect(
    page
      .getByTestId("map-agent-recent-event")
      .filter({ hasText: "Structured completion", hasNotText: "Prompt sent" }),
  ).toContainText("Structured completion");
  await structuredCompletionEvent.click();
  const copiedEvent = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedEvent.toBe(
    `${PRIMARY_MISSION}: signal · Structured completion - Provider emitted a machine-readable completion signal.`,
  );
  await expect(page.getByTestId("canvas-agent-lane-memory")).toContainText(
    "Copy memory",
  );
  await expect(page.getByTestId("canvas-agent-lane-memory")).toContainText(
    "Checkout flake isolated to retry timing",
  );
  await expect(page.getByTestId("map-agent-lane-memory")).toContainText(
    "Checkout flake isolated to retry timing",
  );
  await page.getByTestId("canvas-agent-lane-memory").click();
  const copiedMemory = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedMemory.toBe(
    `${PRIMARY_MISSION}: Checkout flake isolated to retry timing; preserve auth fixture logs.`,
  );
  ...
  await expect(
    page
      .getByLabel("Agent output details")
      .getByText("tests: checkout-flow.spec passed", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent output details")
      .getByText("reports/checkout-flow.md", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByLabel("Agent provider control surface")
  ...
  await expect(
    page
      .getByLabel("Agent provider control surface")
      .getByText("low residual risk", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText("structured", { exact: true })).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("Exit", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Agent run record").getByText("0", { exact: true }),
  ).toBeVisible();
  await expect(page.getByTestId("canvas-agent-lane-attention")).toContainText(
    "Complete",
  );
  await expect(page.getByTestId("canvas-agent-lane-attention")).toContainText(
    "Reviewing checkout report",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-attention-breakdown"),
  ).toContainText("Complete: 1");
  await expect(
    page.getByTestId("canvas-agent-lane-closeout-breakdown"),
  ).toContainText("Ready: 1");
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "1 queue",
  );
  await expect(page.getByTestId("canvas-agent-attention-item")).toContainText(
  ...
  const copiedBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedBrief.toContain(`Agent run: ${PRIMARY_MISSION}`);
  await copiedBrief.toMatch(
    /Run: codex-[a-z0-9]+-[a-z0-9]{6} \(generation 1\)/,
  );
  ...
  await copiedBrief.toContain(
    "Isolation note: Agent shares the selected workspace checkout.",
  );
  await copiedBrief.toContain("Worktree: unknown");
  await copiedBrief.toContain("Worktree cleanup: not-needed");
  await copiedBrief.toContain(
    "Worktree cleanup note: Shared workspace runs do not own a cleanup target.",
  );
  ...
  await copiedBrief.toContain(
    "Run record: prompts=9, sent=9, signals=2, controls=3",
  );
  await copiedBrief.toContain("Latest event: signal - Structured completion");
  await page
    .getByTestId("canvas-agent-run-item")
    .filter({ hasText: "Reviewing checkout report" })
    .click();
  const copiedRunFromLane = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  ...
  await copiedRunFromLane.toContain(
    "Evidence: tests: checkout-flow.spec passed",
  );
  await copiedRunFromLane.toContain(
    "Run record: prompts=9, sent=9, signals=2, controls=3",
  ...
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedLaneBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  ...
  await copiedLaneBrief.toContain(
    `${PRIMARY_MISSION}: sent - [[TERMFLEET_AGENT_EVENT`,
  );
  ...
  await copiedLaneBrief.toContain(
    `${PRIMARY_MISSION}: Review structured result`,
  );
  await copiedLaneBrief.toContain("Mission control:");
  await copiedLaneBrief.toContain(
    "- Investigate flaky checkout flow: Review (Ready with proof · Memory ready · Structured task completed)",
  );
  await copiedLaneBrief.toContain("Recent events:");
  await copiedLaneBrief.toContain(
    "Investigate flaky checkout flow: signal · Structured completion - Provider emitted a machine-readable completion signal.",
  );
  await copiedLaneBrief.toContain("Evidence queue:");
  await copiedLaneBrief.toContain(
    "tests: checkout-flow.spec passed (reports/checkout-flow.md)",
  );
  ...
  await copiedLaneBrief.toContain(
    "Ready with proof · Memory ready · Structured task completed (reports/checkout-flow.md)",
  );
  ...
  await expect(
    page
      .getByLabel("Agent operator guidance")
      .getByText("Workstream reviewed", { exact: true }),
  ).toBeVisible();
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "1 complete",
  );
...
test("command palette can launch a headless Codex agent profile", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  await expect(
    page.getByTestId("map-agent-run-item").filter({ hasText: mission }),
  ).toBeVisible();
  await expect(
    page.getByTestId("map-agent-run-status").filter({ hasText: "codex" }),
  ).toContainText("command is not available in browser preview");

  await page.getByText("Details").click();
  await expect(
    page.getByLabel("Agent provider control surface").getByText("Launch"),
  ).toBeVisible();
  await expect(page.getByLabel("Agent provider control surface")).toContainText(
    "Codex headless status stream",
  );
    ...
    .poll(async () =>
      page.evaluate((mission) => {
        ...
        return {
          ...
          startupCommand: agent?.workstream?.startupCommand,
          controlProtocol: agent?.workstream?.controlProtocol,
          providerEvent: agent?.workstream?.events?.find(
            (event: { kind?: string }) => event.kind === "provider",
          )?.detail,
        };
      }, mission),
    )
...
test("agent cockpit saves operator memory without sending a prompt", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  const memoryNote =
    "Operator note: production deploy waits for DB owner approval.";
  const composer = page.getByRole("textbox", {
    name: "Agent follow-up prompt",
  });
  ...
  await expect
    .poll(async () =>
      page.evaluate((memoryNote) => {
        const raw = localStorage.getItem("terminal-workspace.v1");
        const state = raw ? JSON.parse(raw) : null;
        const agent = state?.tabs?.find(
          (tab: { workstream?: { kind?: string; mission?: string } }) =>
            tab.workstream?.kind === "agent" &&
            tab.workstream?.mission === "Document deploy handoff",
        );
        return {
          memory: agent?.workstream?.memory,
          promptCount: agent?.workstream?.promptCount,
          ...
          outcome: agent?.workstream?.outcome,
          latestInput: agent?.workstream?.inputQueue?.at(-1)?.text,
          hasMemoryEvent: agent?.workstream?.events?.some(
...
test("agent cockpit surfaces raw provider process exit", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  await expect
    .poll(async () =>
      page.evaluate((exitMission) => {
        const raw = localStorage.getItem("terminal-workspace.v1");
        const state = raw ? JSON.parse(raw) : null;
        const agent = state?.tabs?.find(
          (tab: { workstream?: { kind?: string; mission?: string } }) =>
            tab.workstream?.kind === "agent" &&
            tab.workstream?.mission === exitMission,
        );
        const terminal = agent?.terminals?.[0];
        const hasExitEvent = agent?.workstream?.events?.some(
          (event: {
            kind?: string;
            label?: string;
            detail?: string;
    ...
    .toEqual({
      ...
      nextAction: "Inspect output and send recovery prompt",
      outcome: expect.stringMatching(
        /^Provider process exited(?: with code 7)?$/,
      ),
      completedAt: undefined,
      hasExitEvent: true,
    });
  ...
  await expect(
    page.getByLabel("Agent run record").getByText("7", { exact: true }),
  ).toBeVisible();
  await expect(page.getByTestId("canvas-agent-lane-health")).toContainText(
    "Needs attention",
  );
...
test("agent lane sweeps active child agents for status", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  await page.evaluate(
    ({ heldMission, recoveryMission }) => {
      ...
      if (!store) throw new Error("TermFleet test store is unavailable");
      const state = store.getState();
      const held = state.tabs.find(
        (tab) =>
          tab.workstream?.kind === "agent" &&
          tab.workstream?.mission === heldMission,
      );
      const recovery = state.tabs.find(
        (tab) =>
          tab.workstream?.kind === "agent" &&
          tab.workstream?.mission === recoveryMission,
      );
      if (!held?.workstream)
        throw new Error(`Held workstream not found: ${heldMission}`);
      if (!recovery?.workstream)
        throw new Error(`Recovery workstream not found: ${recoveryMission}`);
      const now = Date.now();
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-status-sweep"),
  ).toBeVisible();
  await expect(page.getByTestId("map-agent-lane-status-sweep")).toBeVisible();
  await expect(
    page.getByTestId("canvas-agent-lane-status-sweep-plan"),
  ).toHaveText("Sweep 2 active · 2 held");
  await expect(page.getByTestId("map-agent-lane-status-sweep-plan")).toHaveText(
    "Sweep 2 active · 2 held",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-interrupt-active"),
  ).toBeVisible();
  await expect(
    page.getByTestId("map-agent-lane-interrupt-active"),
  ).toBeVisible();
  await expect(page.getByTestId("canvas-agent-lane-interrupt-plan")).toHaveText(
    "Interrupt 2 active · 2 held",
  );
  await expect(page.getByTestId("map-agent-lane-interrupt-plan")).toHaveText(
    "Interrupt 2 active · 2 held",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-restart-recovery"),
  ).toBeVisible();
  await expect(
    page.getByTestId("map-agent-lane-restart-recovery"),
  ).toBeVisible();
  await expect(page.getByTestId("canvas-agent-lane-restart-plan")).toHaveText(
    "Restart 1 recovery · 3 held",
  );
  await expect(page.getByTestId("map-agent-lane-restart-plan")).toHaveText(
    "Restart 1 recovery · 3 held",
  );
  ...
  await expect
    .poll(async () =>
      page.evaluate(
        ({ activeMissions, heldMission, recoveryMission }) => {
          const raw = localStorage.getItem("terminal-workspace.v1");
          const state = raw ? JSON.parse(raw) : null;
          const activeAgents = activeMissions.map((mission) => {
            ...
            return {
              mission,
              promptCount: agent?.workstream?.promptCount,
              latestHasStatusCheck: latest?.text?.includes(
                "Status check for Codex agent",
              ),
              latestHasMission: latest?.text?.includes(`Mission: ${mission}`),
              latestSource: latest?.source,
              latestLabel: latest?.label,
              hasQueuedEvent: agent?.workstream?.events?.some(
          ...
          return {
            ...
            held: {
              ...
              hasStatusSweepEvent: held?.workstream?.events?.some(
                (event: { label?: string }) =>
                  event.label === "Mission control queued Status sweep",
              ),
            },
            recovery: {
              ...
              hasStatusSweepEvent: recovery?.workstream?.events?.some(
                (event: { label?: string }) =>
                  event.label === "Mission control queued Status sweep",
              ),
            },
        ...
        ({ activeMissions, heldMission, recoveryMission }) => {
          ...
          const activeAgents = activeMissions.map((mission) => {
            ...
            return {
              ...
              nextAction: agent?.workstream?.nextAction,
              controlCount: agent?.workstream?.controlCount,
              hasCancellationEvent: agent?.workstream?.events?.some(
                (event: { label?: string }) =>
          ...
          return {
            ...
            held: {
              ...
              hasCancellationEvent: held?.workstream?.events?.some(
                (event: { label?: string }) =>
                  event.label === "Cancellation requested",
              ),
            },
            recovery: {
              ...
              hasCancellationEvent: recovery?.workstream?.events?.some(
                (event: { label?: string }) =>
                  event.label === "Cancellation requested",
              ),
            },
  ...
  await expect
    ...
    .toEqual({
      ...
      recovery: {
        ...
        nextAction: expect.stringMatching(
          /^(Watch provider startup|Watch provider response)$/,
        ),
        generation: 1,
        controlCount: 1,
        lastEvent: expect.stringMatching(
          /^(Mission control requested restart|Status changed to running|Provider session ready)$/,
        ),
      },
    });

  await expect(page.getByTestId("canvas-agent-lane-restart-plan")).toHaveText(
    "Restart 0 recovery · 4 held",
  );
  await expect(page.getByTestId("map-agent-lane-restart-plan")).toHaveText(
    "Restart 0 recovery · 4 held",
  );
...
test("agent lane flags completed work without proof", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  await expect(page.getByTestId("canvas-agent-lane-proof-plan")).toHaveText(
    "Proof 1 needed",
  );
  await expect(page.getByTestId("map-agent-lane-proof-plan")).toHaveText(
    "Proof 1 needed",
  );
  await expect(page.getByTestId("canvas-agent-lane-memory-plan")).toHaveText(
    "Memory 0 needed · 1 held",
  );
  await expect(page.getByTestId("map-agent-lane-memory-plan")).toHaveText(
    "Memory 0 needed · 1 held",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-review-ready"),
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-request-proof"),
  ).toBeEnabled();
  await expect(page.getByTestId("map-agent-lane-request-proof")).toBeEnabled();
  await expect(
    page.getByTestId("canvas-agent-lane-request-memory"),
  ).toBeDisabled();
  await expect(
    page.getByTestId("map-agent-lane-request-memory"),
  ).toBeDisabled();
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "1 mission",
  );
  await expect(page.getByTestId("canvas-agent-working-on")).toContainText(
    "Summarize flaky test failures",
  );
  await expect(page.getByTestId("canvas-agent-status-now")).toContainText(
    "Ready for review",
  );
  ...
  await expect(page.getByTestId("canvas-agent-proof-item")).toContainText(
    "Request proof",
  );
  await expect(page.getByTestId("canvas-agent-proof-item")).toContainText(
    "Summarize flaky test failures",
  ...
  await expect(page.getByTestId("canvas-agent-proof-item")).toContainText(
    "Ask for exact tests and artifact paths",
  );
  await expect(page.getByTestId("canvas-agent-review-item")).toContainText(
    "Needs proof",
  );
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-closeout-breakdown"),
  ).toContainText("Needs proof + memory: 1");
  await expect(
    page.getByRole("button", { name: "Mark run reviewed", exact: true }),
  ).toBeDisabled();
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          phase: agent?.workstream?.phase,
          reviewedAt: agent?.workstream?.reviewedAt,
          hasReviewedEvent: agent?.workstream?.events?.some(
            (event: { label?: string }) =>
              event.label === "Mission control reviewed run" ||
              event.label === "Reviewed",
          ),
        };
      }),
    )
  ...
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Request proof" }),
  ).toContainText("Summarize flaky test failures");
  await expect(page.getByTestId("map-agent-lane-headline")).toContainText(
    "Next: Request proof",
  );
  await expect(
    page
      .getByTestId("map-agent-supervisor-item")
      .filter({ hasText: "Request proof" }),
  ).toContainText("Summary finished");
  await expect(page.getByTestId("map-agent-proof-item")).toContainText(
    "Request proof",
  );
  await expect(page.getByTestId("map-agent-proof-item")).toContainText(
    "Summary finished",
  ...
  await expect(
    page.getByTestId("map-agent-lane-closeout-breakdown"),
  ).toContainText("Needs proof + memory: 1");
  await expect(page.getByTestId("canvas-agent-evidence-item")).toHaveCount(0);

  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedProofBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedProofBrief.toContain(
    "Cockpit headline: Next: Request proof - Summarize flaky test failures · Summary finished",
  );
  await copiedProofBrief.toContain("Mission control:");
  await copiedProofBrief.toContain(
    "- Summarize flaky test failures: Request proof (Summary finished)",
  );
  await copiedProofBrief.toContain("Proof needed:");
  await copiedProofBrief.toContain(
    "- Summarize flaky test failures: Summary finished",
  );
  await copiedProofBrief.toContain(
    "Request: Ask for exact tests and artifact paths",
  );
  ...
  await expect
    ...
    .toEqual({
      promptCount: 3,
      latestInput: expect.stringContaining(
        "Provide proof for Codex agent completion",
      ),
      latestInputSent: true,
      latestInputSource: "mission-control",
  ...
  await expect
    ...
    .toContain(
      "Current summary: Summary finished. Operator request: Ask for exact tests and artifact paths",
    );
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          nextAction: agent?.workstream?.nextAction,
          signalCount: agent?.workstream?.signalCount,
          hasProofEvent: agent?.workstream?.events?.some(
            (event: { kind?: string; label?: string }) =>
  ...
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Request memory" }),
  ).toContainText("Summary finished with proof");
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Request memory" }),
  ...
  await expect(page.getByTestId("canvas-agent-evidence-item")).toContainText(
    "npm test -- flaky-checkout passed",
  );
  await expect(page.getByTestId("map-agent-evidence-item")).toContainText(
    "reports/flaky-checkout-summary.md",
  );
  await expect(
    page.getByRole("button", { name: "Draft proof request" }),
  ).toHaveCount(0);
  await page.getByTestId("canvas-agent-evidence-item").click();
  const copiedEvidence = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedEvidence.toBe(
    "Summarize flaky test failures: npm test -- flaky-checkout passed (reports/flaky-checkout-summary.md)",
  );
  await expect
    .poll(async () =>
      page.evaluate(() => {
  ...
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedResolvedProofBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  ...
  await copiedResolvedProofBrief.toContain(
    "npm test -- flaky-checkout passed (reports/flaky-checkout-summary.md)",
  );
  await copiedResolvedProofBrief.toContain("Handoff memory needed:");
  await copiedResolvedProofBrief.toContain(
    "Request: Provide durable handoff memory",
  );
  await copiedResolvedProofBrief.toContain(
    "Ready with proof · Needs memory · Summary finished with proof (reports/flaky-checkout-summary.md)",
  );
  await copiedResolvedProofBrief.toContain("Closeout mix: Needs memory: 1");
  await copiedResolvedProofBrief.toContain(
    "0 closeout ready · 1 closeout blocked",
  );
  await copiedResolvedProofBrief.toContain(
    "0 handoff ready · 1 handoff missing",
  );

  await page.getByTestId("canvas-agent-lane-copy-mission").click();
  const copiedMemoryMissionBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  ...
  await copiedMemoryMissionBrief.toContain(
    "- Summarize flaky test failures: Request memory (Ready with proof · Needs memory · Summary finished with proof)",
  );
  ...
  await copiedMemoryMissionBrief.toContain(
    "Prompt: Provide durable handoff memory",
  );
  ...
  await page.evaluate(() => {
    ...
    const agent = state.tabs.find(
      (tab) =>
        tab.workstream?.kind === "agent" &&
        tab.workstream?.mission === "Summarize flaky test failures",
    );
    if (!agent) throw new Error("Agent workstream not found");
    state.recordWorkstreamMemory(
      agent.id,
      "Proof lives in reports/flaky-checkout-summary.md; retry timing is the remaining context.",
    );
  });
  await expect(page.getByTestId("canvas-agent-lane-memory-plan")).toHaveText(
    "Memory 0 needed · 1 held",
...
test("agent lane exposes hidden mission-control queue pressure", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  const overflowMissions = Array.from(
    { length: 6 },
    (_, index) => `Overflow proof ${index + 1}`,
  );
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-mission-breakdown"),
  ).toContainText("Request proof: 6");
  await expect(
    page.getByTestId("canvas-agent-lane-mission-breakdown"),
  ).toContainText("Review: 6");
  await expect(page.getByTestId("canvas-agent-supervisor-item")).toHaveCount(5);
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("Codex · done/complete");
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("shared workspace");
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("Now: Ready for proof");
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("Signal: just now");
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("Source: complete · structured");
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("Also: Review");
  await expect(
    page.getByTestId("canvas-agent-supervisor-item").first(),
  ).toContainText("Needs proof · Memory ready");
  await expect(
    page.getByTestId("canvas-agent-supervisor-overflow"),
  ).toContainText("+1 rows · 3 actions");
  await expect(
    page.getByTestId("canvas-agent-supervisor-overflow"),
  ).toContainText("Overflow proof 6");
  await expect(
    page.getByTestId("canvas-agent-supervisor-overflow"),
  ).toContainText("Request proof");
  await expect(
    page.getByTestId("canvas-agent-supervisor-overflow"),
  ).toContainText("Needs proof 6");
  await expect(
    page.getByTestId("canvas-agent-supervisor-overflow"),
  ).toContainText("Also: Review");
  await expect(
    page.getByTestId("canvas-agent-supervisor-overflow"),
  ).toContainText("Complete: Ready for proof");
  await expect(page.getByTestId("canvas-agent-run-item")).toHaveCount(3);
  await expect(page.getByTestId("canvas-agent-run-overflow")).toContainText(
    "+3 more agents",
  );
  await expect(page.getByTestId("canvas-agent-run-overflow")).toContainText(
    "Ready for proof",
  ...
  await expect(page.getByTestId("canvas-agent-input-overflow")).toContainText(
    "more prompts",
  );
  await expect(page.getByTestId("canvas-agent-input-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(
    page.getByTestId("canvas-agent-recent-event-overflow"),
  ).toContainText("more events");
  await expect(
    page.getByTestId("canvas-agent-recent-event-overflow"),
  ).toContainText("Overflow proof");
  await expect(
    page.getByTestId("canvas-agent-recent-event-overflow"),
  ).toContainText(/Provider session ready|Status changed to running/);
  await expect(page.getByTestId("canvas-agent-output-item")).toHaveCount(3);
  await expect(page.getByTestId("canvas-agent-output-overflow")).toContainText(
    "more output",
  );
  await expect(page.getByTestId("canvas-agent-output-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("canvas-agent-next-overflow")).toContainText(
    "more next",
  );
  await expect(page.getByTestId("canvas-agent-next-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("canvas-agent-proof-overflow")).toContainText(
    "more proof",
  );
  await expect(page.getByTestId("canvas-agent-proof-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("canvas-agent-review-overflow")).toContainText(
    "more review",
  );
  await expect(page.getByTestId("canvas-agent-review-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("canvas-agent-memory-overflow")).toContainText(
    "more memory",
  );
  await expect(page.getByTestId("canvas-agent-memory-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(
    page.getByTestId("map-agent-lane-mission-breakdown"),
  ).toContainText("Request proof: 6");
  await expect(
    page.getByTestId("map-agent-lane-mission-breakdown"),
  ).toContainText("Review: 6");
  ...
  await expect(
    page.getByTestId("map-agent-supervisor-item").first(),
  ).toContainText("Now: Ready for proof");
  await expect(
    page.getByTestId("map-agent-supervisor-item").first(),
  ).toContainText("Signal: just now");
  ...
  await expect(page.getByTestId("map-agent-supervisor-overflow")).toContainText(
    "Overflow proof 6",
  );
  await expect(page.getByTestId("map-agent-supervisor-overflow")).toContainText(
    "Request proof",
  ...
  await expect(page.getByTestId("map-agent-supervisor-overflow")).toContainText(
    "Complete: Ready for proof",
  );
  await expect(page.getByTestId("map-agent-run-item")).toHaveCount(3);
  await expect(page.getByTestId("map-agent-run-overflow")).toContainText(
    "+3 more agents",
  );
  await expect(page.getByTestId("map-agent-run-overflow")).toContainText(
    "Ready for proof",
  ...
  await expect(page.getByTestId("map-agent-input-overflow")).toContainText(
    "more prompts",
  );
  await expect(page.getByTestId("map-agent-input-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(
    page.getByTestId("map-agent-recent-event-overflow"),
  ).toContainText("more events");
  await expect(
    page.getByTestId("map-agent-recent-event-overflow"),
  ).toContainText("Overflow proof");
  ...
  await expect(page.getByTestId("map-agent-output-overflow")).toContainText(
    "more output",
  );
  await expect(page.getByTestId("map-agent-output-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-next-overflow")).toContainText(
    "more next",
  );
  await expect(page.getByTestId("map-agent-next-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-proof-overflow")).toContainText(
    "more proof",
  );
  await expect(page.getByTestId("map-agent-proof-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-review-overflow")).toContainText(
    "more review",
  );
  await expect(page.getByTestId("map-agent-review-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-memory-overflow")).toContainText(
    "more memory",
  );
  await expect(page.getByTestId("map-agent-memory-overflow")).toContainText(
    "Overflow proof",
  ...
  await page.getByTestId("canvas-agent-lane-copy-mission").click();
  const copiedMissionBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedMissionBrief.toContain("Agent mission control brief");
  await copiedMissionBrief.toContain(
    "Queue: 6 mission rows · 1 hidden rows · 18 actions · 3 hidden actions · 6 agents",
  );
  ...
  await copiedMissionBrief.toContain(
    "Dispatch: 0 mission-control prompts · 0 sent · 0 queued",
  );
  await copiedMissionBrief.toContain(
    "Breakdown: Request proof: 6 · Review: 6 · Complete: 6",
  );
  await copiedMissionBrief.toContain(
    "Headline: Next: Request proof - Overflow proof",
  );
  await copiedMissionBrief.toContain("Run: Codex · done/complete");
  await copiedMissionBrief.toContain(
    "Workspace: workspace root unknown · branch unknown · state unknown · shared workspace",
  );
  ...
  await copiedMissionBrief.toContain(
    "Also: Review: Needs proof · Memory ready",
  );
  await copiedMissionBrief.toContain("Prompt: Attach proof");
  await copiedMissionBrief.toContain(
    "- +1 more mission rows hidden (3 actions)",
  );
  await copiedMissionBrief.toContain("Hidden mission control:");
  await copiedMissionBrief.toContain(
    "- Overflow proof 6: Request proof (Needs proof 6)",
  );
  await copiedMissionBrief.toContain(
    "Also: Review: Needs proof · Memory ready",
  );
  ...
  const copiedMapMissionBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedMapMissionBrief.toContain("Agent mission control brief");
  await copiedMapMissionBrief.toContain(
    "Queue: 6 mission rows · 1 hidden rows · 18 actions · 3 hidden actions · 6 agents",
  );
  ...
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedOverflowBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedOverflowBrief.toContain(
    "6 mission rows · 1 hidden mission rows · 18 mission actions · 3 hidden mission actions",
  );
  ...
  await expect(
    page.getByTestId("canvas-agent-evidence-overflow"),
  ).toContainText("more evidence");
  await expect(
    page.getByTestId("canvas-agent-evidence-overflow"),
  ).toContainText("Overflow proof");
  await expect(
    page.getByTestId("canvas-agent-evidence-overflow"),
  ).toContainText("Verification evidence");
  await expect(
    page.getByTestId("canvas-agent-evidence-overflow"),
  ).toContainText("reports/overflow-proof");
  await expect(page.getByTestId("map-agent-evidence-item")).toHaveCount(3);
  await expect(page.getByTestId("map-agent-evidence-overflow")).toContainText(
    "more evidence",
  );
  await expect(page.getByTestId("map-agent-evidence-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-auth-retry-plan"),
  ).toHaveText("Retry 6 auth");
  await expect(page.getByTestId("map-agent-lane-auth-retry-plan")).toHaveText(
    "Retry 6 auth",
  );
  ...
  await expect(page.getByTestId("canvas-agent-auth-overflow")).toContainText(
    "more auth",
  );
  await expect(page.getByTestId("canvas-agent-auth-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-auth-overflow")).toContainText(
    "more auth",
  );
  await expect(page.getByTestId("map-agent-auth-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("canvas-agent-lane-risk-plan")).toHaveText(
    "Risk 6 open",
  );
  await expect(page.getByTestId("map-agent-lane-risk-plan")).toHaveText(
    "Risk 6 open",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-mitigate-risk"),
  ).toBeEnabled();
  ...
  await expect(page.getByTestId("canvas-agent-risk-overflow")).toContainText(
    "more risk",
  );
  await expect(page.getByTestId("canvas-agent-risk-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-risk-overflow")).toContainText(
    "more risk",
  );
  await expect(page.getByTestId("map-agent-risk-overflow")).toContainText(
    "Overflow proof",
      ...
      page.evaluate((missions) => {
        ...
        const agents =
          state?.tabs?.filter(
            (tab: { workstream?: { kind?: string; mission?: string } }) =>
              tab.workstream?.kind === "agent" &&
              missions.includes(tab.workstream.mission ?? ""),
          ) ?? [];
        return agents
          .map(
            (tab: {
              ...
              return {
                ...
                hasQueuedEvent: tab.workstream?.events?.some(
                  (event) =>
                    event.label === "Mission control queued Mitigate risk",
                ),
              };
          ...
          .sort((a: { mission?: string }, b: { mission?: string }) =>
            (a.mission ?? "").localeCompare(b.mission ?? ""),
          );
      }, overflowMissions),
    )
    .toEqual(
      overflowMissions.map((mission) => ({
        mission,
        latestInput: expect.stringContaining("Resolve risk for Codex agent"),
        latestInputSource: "mission-control",
        latestInputLabel: "Mitigate risk",
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-dispatch-breakdown"),
  ).toContainText("Mitigate risk: 6 sent");
  await expect(page.getByTestId("canvas-agent-run-item").first()).toContainText(
    "Ask: Mitigate risk · sent",
  );
  await expect(page.getByTestId("canvas-agent-run-item").first()).toContainText(
    "Resolve risk for Codex agent",
  ...
  await expect(
    page.getByTestId("sidebar-agent-lane-dispatch-breakdown"),
  ).toContainText("Mitigate risk: 6 sent");
  await expect(
    page.getByTestId("sidebar-agent-run-item").first(),
  ).toContainText("Ask: Mitigate risk · sent");
  await expect(
    page.getByTestId("sidebar-agent-run-item").first(),
  ).toContainText("Resolve risk for Codex agent");
  await page.getByRole("button", { name: "Map", exact: true }).click();
  await expect(
    page.getByTestId("map-agent-lane-dispatch-breakdown"),
  ).toContainText("Mitigate risk: 6 sent");
  await expect(page.getByTestId("map-agent-run-item").first()).toContainText(
    "Ask: Mitigate risk · sent",
  );
  await expect(page.getByTestId("map-agent-run-item").first()).toContainText(
    "Resolve risk for Codex agent",
  ...
  const copiedRiskRunBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedRiskRunBrief.toContain(
    "Cockpit ask: Ask: Mitigate risk · sent · Resolve risk for Codex agent",
  );
  await copiedRiskRunBrief.toContain(
    "Latest input: sent via mission-control Mitigate risk - Resolve risk for Codex agent",
  );
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedRiskLaneBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedRiskLaneBrief.toContain("Agent runs:");
  await copiedRiskLaneBrief.toContain(
    "Cockpit ask: Ask: Mitigate risk · sent · Resolve risk for Codex agent",
  );
  await page.getByTestId("canvas-agent-lane-copy-mission").click();
  const copiedRiskMissionBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedRiskMissionBrief.toContain(
    "Dispatch: 6 mission-control prompts · 6 sent · 0 queued",
  );
  ...
  await expect(
    page.getByTestId("canvas-agent-recovery-overflow"),
  ).toContainText("more recovery");
  await expect(
    page.getByTestId("canvas-agent-recovery-overflow"),
  ).toContainText("Overflow proof");
  await expect(
    page.getByTestId("canvas-agent-recovery-overflow"),
  ).toContainText("Provider failure");
  await expect(
    page.getByTestId("canvas-agent-recovery-overflow"),
  ).toContainText("Recover Codex agent");
  await expect(page.getByTestId("map-agent-recovery-item")).toHaveCount(3);
  await expect(page.getByTestId("map-agent-recovery-overflow")).toContainText(
    "more recovery",
  );
  await expect(page.getByTestId("map-agent-recovery-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(
    page.getByTestId("canvas-agent-attention-overflow"),
  ).toContainText("more attention");
  await expect(
    page.getByTestId("canvas-agent-attention-overflow"),
  ...
  await expect(
    page.getByTestId("canvas-agent-attention-overflow"),
  ).toContainText("Recovery needed");
  await expect(page.getByTestId("map-agent-attention-item")).toHaveCount(3);
  await expect(page.getByTestId("map-agent-attention-overflow")).toContainText(
    "more attention",
  );
  await expect(page.getByTestId("map-agent-attention-overflow")).toContainText(
    "Blocked",
  ...
  await expect(page.getByTestId("canvas-agent-stale-overflow")).toContainText(
    "more stale",
  );
  await expect(page.getByTestId("canvas-agent-stale-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(page.getByTestId("map-agent-stale-overflow")).toContainText(
    "more stale",
  );
  await expect(page.getByTestId("map-agent-stale-overflow")).toContainText(
    "Overflow proof",
  ...
  await expect(
    page.getByTestId("canvas-agent-workspace-group-overflow"),
  ).toContainText("more groups");
  await expect(
    page.getByTestId("canvas-agent-workspace-group-overflow"),
  ).toContainText("Workspace group");
  await expect(
    page.getByTestId("canvas-agent-workspace-group-overflow"),
  ).toContainText("1 agents");
  await expect(
    page.getByTestId("canvas-agent-workspace-group-overflow"),
  ).toContainText("1 active");
  await expect(
    page.getByTestId("canvas-agent-workspace-group-overflow"),
  ).toContainText("branch-overflow");
  await expect(page.getByTestId("map-agent-workspace-group")).toHaveCount(3);
  await expect(
    page.getByTestId("map-agent-workspace-group-overflow"),
  ).toContainText("more groups");
  await expect(
    page.getByTestId("map-agent-workspace-group-overflow"),
  ).toContainText("Workspace group");
  ...
  await expect
    ...
    .toEqual(
      overflowMissions.map((mission) => ({
        ...
        readiness: "unknown",
        readinessCheck:
          "Restart requested; waiting for provider startup output",
        currentActivity: expect.any(String),
        latestEvent: expect.stringMatching(
          /^(Mission control requested restart|Status changed to running)$/,
        ),
      })),
    );
  await expect(
    page.getByTestId("canvas-agent-lane-auth-retry-plan"),
  ).toHaveText("Retry 0 auth · 6 held");
  await expect(page.getByTestId("map-agent-lane-auth-retry-plan")).toHaveText(
    "Retry 0 auth · 6 held",
  );
...
test("agent lane requests cleanup only for closeout-ready dedicated worktrees", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  await expect(page.getByTestId("canvas-agent-lane-cleanup-plan")).toHaveText(
    "Cleanup 1 ready · 2 held",
  );
  await expect(page.getByTestId("map-agent-lane-cleanup-plan")).toHaveText(
    "Cleanup 1 ready · 2 held",
  );
  await expect(
    page.getByTestId("canvas-agent-lane-request-cleanup"),
  ).toBeEnabled();
  await expect(
    page.getByTestId("map-agent-lane-request-cleanup"),
  ).toBeEnabled();
  await expect(page.getByTestId("canvas-agent-lane-health")).toContainText(
    "Needs attention",
  );
        ...
        return agents
          .map(
            (tab: {
              ...
              hasCleanupRequestedEvent:
                tab.workstream?.events?.some(
                  (event) =>
                    event.label ===
                      "Mission control requested worktree cleanup" &&
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-cleanup-breakdown"),
  ).toContainText("requested: 1");
  await expect(page.getByTestId("map-agent-lane-health")).toContainText(
    "1 cleanup",
  );
  await expect(page.getByTestId("canvas-agent-lane-health")).toContainText(
  ...
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Cleanup pending" }),
  ).toContainText("Finished isolated cleanup");
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Cleanup pending" }),
  ).toContainText("Cleanup requested for /tmp/termfleet-finished-cleanup");
  await expect(
    page
      .getByTestId("map-agent-supervisor-item")
      .filter({ hasText: "Cleanup pending" }),
  ...
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedCleanupMissionBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedCleanupMissionBrief.toContain("Mission control:");
  await copiedCleanupMissionBrief.toContain(
    "- Finished isolated cleanup: Cleanup pending (Cleanup requested for /tmp/termfleet-finished-cleanup)",
  );
  await copiedCleanupMissionBrief.toContain(
    "Action: focus run and execute guarded cleanup",
  );
});

test("agent lane summarizes multiple supervised workstreams", async ({
  page,
  context,
}) => {
  ...
  await expect(
    page
      .getByTestId("map-agent-run-item")
      .filter({ hasText: "Prepare release notes" }),
  ).toContainText("command is not available in browser preview");
  ...
  const mapCodexRun = page
    .getByTestId("map-agent-run-item")
    .filter({ hasText: "Audit deployment scripts" });
  const mapClaudeRun = page
    .getByTestId("map-agent-run-item")
    .filter({ hasText: "Prepare release notes" });
  await expect(mapCodexRun.getByTestId("map-agent-run-status")).toContainText(
    "working · codex · Auditing deployment scripts",
  );
  await expect(mapCodexRun.getByTestId("map-agent-run-status")).toContainText(
    "Attach proof",
  );
  await expect(mapClaudeRun.getByTestId("map-agent-run-status")).toContainText(
    "idle · claude · Waiting for operator follow-up",
  );
  await expect(mapClaudeRun.getByTestId("map-agent-run-status")).toContainText(
  ...
  await expect(
    page
      .getByTestId("canvas-agent-working-on")
      .filter({ hasText: "Audit deployment scripts" }),
  ).toBeVisible();
  await expect(
    page.getByTestId("canvas-agent-status-path").first(),
  ).toContainText("Path");
  await expect(
    page
      .getByTestId("canvas-agent-status-now")
      .filter({ hasText: "Auditing deployment scripts" }),
  ).toBeVisible();
  await expect(
    page.getByTestId("canvas-agent-status-chips").first(),
  ).toContainText("codex");
  await expect(
    page.getByTestId("canvas-agent-status-chips").first(),
  ).toContainText("working");
  await page.getByRole("button", { name: "Sessions", exact: true }).click();
  const sidebarCodexRun = page
    .getByTestId("sidebar-agent-run-item")
    .filter({ hasText: "Audit deployment scripts" });
  const sidebarClaudeRun = page
    .getByTestId("sidebar-agent-run-item")
    .filter({ hasText: "Prepare release notes" });
  await expect(
    sidebarCodexRun.getByTestId("sidebar-agent-run-status"),
  ).toContainText("working · codex · Auditing deployment scripts");
  await expect(
    sidebarClaudeRun.getByTestId("sidebar-agent-run-status"),
  ).toContainText("idle · claude · Waiting for operator follow-up");
  await sidebarClaudeRun.click();
  await expect(page.getByTestId("split-agent-pane-now")).toContainText(
    "Waiting for operator follow-up",
  );
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-isolation-breakdown"),
  ).toContainText("dedicated worktree requested: 1");
  await expect(
    page.getByTestId("map-agent-lane-isolation-breakdown"),
  ).toContainText("shared workspace: 1");
  await expect(
  ...
  await expect(
    page.getByTestId("canvas-agent-lane-cleanup-breakdown"),
  ).toContainText("not-needed: 1");
  await expect(
    page.getByTestId("canvas-agent-lane-cleanup-breakdown"),
  ).toContainText("manual: 1");
  await expect(
    page.getByTestId("map-agent-lane-cleanup-breakdown"),
  ).toContainText("not-needed: 1");
  await expect(
    page.getByTestId("map-agent-lane-cleanup-breakdown"),
  ).toContainText("manual: 1");
  ...
  await expect(
    page
      .getByTestId("map-agent-workspace-group")
      .filter({ hasText: "shared workspace" }),
  ).toContainText("1 agents");
  await expect(
    page
      .getByTestId("map-agent-workspace-group")
      .filter({ hasText: "dedicated worktree requested" }),
  ).toContainText("1 agents");
  await expect(
    page
      .getByTestId("canvas-agent-input-item")
      .filter({ hasText: "Prepare release notes" }),
  ).toContainText("Copy prompt");
  await expect(
    page
      .getByTestId("map-agent-input-item")
      .filter({ hasText: "Audit deployment scripts" }),
  ).toContainText(/queued|sent/);
  await page
    .getByTestId("canvas-agent-input-item")
    .filter({ hasText: "Prepare release notes" })
    .click();
  const copiedPrompt = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedPrompt.toMatch(
    /^Prepare release notes: (queued|sent) - Prepare release notes$/,
  );
  ...
  await copiedLaneBrief.toContain(
    "Totals: 2 agents · 2 workspace groups · 0 mission rows · 0 hidden mission rows · 0 mission actions · 0 hidden mission actions · 2 prompts · 0 mission-control prompts · 0 mission-control sent · 2 outputs · 2 next actions · 0 memories · 5 events · 0 stale · 0 evidence · 0 proof needed · 0 auth · 0 risk · 0 recovery · 0 review ready · 0 closeout ready · 0 closeout blocked · 0 proven review · 0 unproven review · 0 handoff ready · 0 handoff missing · 0 attention queue · 1 active",
  );
  await copiedLaneBrief.toContain(
    "Mission-control dispatch: 0 mission-control prompts · 0 sent · 0 queued",
  );
  ...
  await copiedLaneBrief.toContain(
    "Isolation mix: shared workspace: 1 · dedicated worktree requested: 1",
  );
  await copiedLaneBrief.toContain("Cleanup mix: not-needed: 1 · manual: 1");
  await copiedLaneBrief.toContain(
    "Cockpit headline: Running - 1 active agent · 2 next actions",
  );
  await copiedLaneBrief.toContain("Recent events:");
  ...
  await copiedLaneBrief.toContain(
    "- Audit deployment scripts: sent - Audit deployment scripts",
  );
  await copiedLaneBrief.toContain("Next actions:");
  await copiedLaneBrief.toContain(
    "- Prepare release notes: Ready for next prompt",
  );
  await copiedLaneBrief.toContain(
    "- Audit deployment scripts: Watch provider response",
  );
  await copiedLaneBrief.toContain("Stale agents:");
  await copiedLaneBrief.toContain("- none");
  await copiedLaneBrief.toContain("Workspace groups:");
  await copiedLaneBrief.toContain(
    "- workspace root unknown: 1 agents, 1 active (shared workspace · branch unknown)",
  );
  await copiedLaneBrief.toContain(
    "- workspace root unknown: 1 agents, 0 active (dedicated worktree requested · branch unknown)",
  );
  await copiedLaneBrief.toContain("Mission control:");
  await copiedLaneBrief.toContain("- none");
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        const agents =
          state?.tabs?.filter(
            (tab: {
              workstream?: { kind?: string; status?: string; phase?: string };
            }) => tab.workstream?.kind === "agent",
          ) ?? [];
        return {
          count: agents.length,
          isolationModes: agents
            .map(
              (tab: { workstream?: { isolationMode?: string } }) =>
                tab.workstream?.isolationMode,
            )
            .sort(),
          isolationStatuses: agents
            .map(
              (tab: { workstream?: { isolationStatus?: string } }) =>
                tab.workstream?.isolationStatus,
            )
            .sort(),
          cleanupStatuses: agents
            .map(
              (tab: { workstream?: { worktreeCleanupStatus?: string } }) =>
                tab.workstream?.worktreeCleanupStatus,
            )
            .sort(),
          dedicatedNote: agents.find(
            (tab: {
              workstream?: { isolationMode?: string; isolationNote?: string };
            }) => tab.workstream?.isolationMode === "dedicated-worktree",
          )?.workstream?.isolationNote,
          dedicatedCleanupNote: agents.find(
            (tab: {
              workstream?: {
          ...
          activeLike: agents.filter(
            (tab: { workstream?: { status?: string; phase?: string } }) =>
  ...
  await ageAgentWorkstream(
    page,
    "Audit deployment scripts",
    16,
    "Waiting on deployment audit",
  );
  await expect(page.getByTestId("canvas-agent-lane-summary")).toContainText(
    "1 stale",
  );
  ...
  await expect(page.getByTestId("canvas-agent-stale-item")).toContainText(
    "Audit deployment scripts",
  );
  await expect(page.getByTestId("canvas-agent-stale-item")).toContainText(
    "16m idle",
  ...
  ).toContainText("Audit deployment scripts");
  await expect(
    page
      .getByTestId("canvas-agent-supervisor-item")
      .filter({ hasText: "Check in" }),
  ).toContainText("Signal: 16m ago");
  await expect(
    page
      .getByTestId("map-agent-supervisor-item")
      .filter({ hasText: "Check in" }),
  ).toContainText("16m idle");
  await expect(
    page
      .getByTestId("map-agent-supervisor-item")
      .filter({ hasText: "Check in" }),
  ).toContainText("Signal: 16m ago");
  await expect(page.getByTestId("map-agent-stale-item")).toContainText(
    "Audit deployment scripts",
  );
  await expect(page.getByTestId("map-agent-stale-item")).toContainText(
    "16m idle",
  ...
  await page.getByTestId("canvas-agent-lane-copy-brief").click();
  const copiedStaleBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedStaleBrief.toContain(
    "Totals: 2 agents · 2 workspace groups · 1 mission rows · 0 hidden mission rows · 1 mission actions · 0 hidden mission actions · 2 prompts · 0 mission-control prompts · 0 mission-control sent · 2 outputs · 2 next actions · 0 memories · 5 events · 1 stale",
  );
  await copiedStaleBrief.toContain(
    "Mission-control dispatch: 0 mission-control prompts · 0 sent · 0 queued",
  );
  ...
  await copiedStaleBrief.toContain(
    "Cockpit headline: Next: Check in - Audit deployment scripts · 16m idle · Waiting on deployment audit",
  );
  await copiedStaleBrief.toContain("Mission control:");
  await copiedStaleBrief.toContain(
    "- Audit deployment scripts: Check in (16m idle · Waiting on deployment audit)",
  );
  await copiedStaleBrief.toContain("Signal: 16m ago");
  await copiedStaleBrief.toContain("Stale agents:");
  await copiedStaleBrief.toContain(
    "- Audit deployment scripts: 16m idle · Waiting on deployment audit",
  );
  await page
    .getByTestId("canvas-agent-supervisor-item")
    .filter({ hasText: "Check in" })
  ...
  await expect(
    page.getByRole("button", { name: "Draft status check" }),
  ).toHaveCount(0);
  await expect
    .poll(async () =>
      page.evaluate(() => {
    ...
    .toEqual({
      currentActivity: expect.stringMatching(
        /Status check for Codex agent|command is not available in browser preview/,
      ),
      lastSummary: expect.stringMatching(
        /^(Mission control: Check in sent to provider|Provider is running)$/,
      ),
  ...
  await page
    ...
    .filter({ hasText: "shared workspace" })
    .click();
  const copiedWorkspaceGroup = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedWorkspaceGroup.toBe(
    "workspace root unknown: 1 agents, 1 active (shared workspace · branch unknown)",
  );

  await expect(
    page.getByRole("button", { name: "Request worktree cleanup" }),
  ).toBeVisible();
  await page
    .getByTestId("agent-cockpit-panel")
    .getByText("Details")
  ...
  await expect(
    page
      .getByTestId("map-agent-workspace-group")
      .filter({ hasText: "dedicated worktree requested" }),
  ).toContainText("1 cleanup");
  await page.getByTestId("map-agent-lane-copy-brief").click();
  const copiedCleanupBrief = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedCleanupBrief.toContain(
    "Cleanup mix: not-needed: 1 · requested: 1",
  );
  ...
  await expect
    .poll(async () =>
      page.evaluate(() => {
        const raw = localStorage.getItem("terminal-workspace.v1");
        const state = raw ? JSON.parse(raw) : null;
        const dedicated = state?.tabs?.find(
          (tab: { workstream?: { kind?: string; isolationMode?: string } }) =>
            tab.workstream?.kind === "agent" &&
            tab.workstream?.isolationMode === "dedicated-worktree",
        );
        return {
  ...
  await expect(
    page.getByRole("button", { name: "Execute worktree cleanup" }),
  ).toBeVisible();
...
test("agent lanes render extracted cockpit objects with explicit copy and proof actions", async ({
  page,
  context,
}) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], {
  ...
  await expect(
    page
      .getByTestId("canvas-agent-extracted-item")
      .filter({ hasText: "Blocker · new" }),
  ).toContainText("Auth fixture token expired");
  await expect(
    page
      .getByTestId("canvas-agent-extracted-item")
      .filter({ hasText: "Next · new" }),
  ).toContainText("Rerun checkout-flow.spec with retry trace");
  await expect(
    page
      .getByTestId("canvas-agent-extracted-item")
      .filter({ hasText: "Task · new" }),
  ).toContainText("Persist retry evidence on the checkout report");
  await expect(
    page
      .getByTestId("canvas-agent-extracted-item")
      .filter({ hasText: "Evidence · new" }),
  ).toContainText("npm test -- checkout-flow.spec passed");
  await expect(page.getByTestId("map-agent-extracted-item")).toHaveCount(4);
  await expect(
    page
      .getByTestId("map-agent-extracted-item")
      .filter({ hasText: "Blocker · new" }),
  ).toContainText("Auth fixture token expired");
  ...
  await expect(
    page
      .getByTestId("sidebar-agent-extracted-item")
      .filter({ hasText: "Task · new" }),
  ).toContainText("Persist retry evidence on the checkout report");
  await runWorkspaceCommand(page, "show map");
  ...
  await expect
    .poll(async () =>
      page.evaluate(() => {
        ...
        return (
          agent?.workstream?.inputQueue?.filter(
            (item: { label?: string }) => item.label === "Request proof",
          ).length ?? 0
        );
      }),
    )
  ...
  await page
    .getByTestId("canvas-agent-extracted-item")
    .filter({ hasText: "Rerun checkout-flow.spec with retry trace" })
    .getByRole("button", { name: "Convert Next to prompt" })
    .click();
  await expect
    .poll(async () =>
      page.evaluate(() => {
        ...
        const prompt = agent?.workstream?.inputQueue?.find(
          (item: { label?: string; text?: string }) =>
            item.label === "Object prompt",
        );
        const next = agent?.workstream?.cockpitObjects?.find(
          (item: { text?: string }) =>
            item.text === "Rerun checkout-flow.spec with retry trace",
        );
        return {
          prompt: prompt?.text ?? "",
  ...
  await page
    .getByTestId("canvas-agent-extracted-item")
    .filter({ hasText: "Rerun checkout-flow.spec with retry trace" })
    .getByRole("button", { name: "Copy Next" })
    .click();
  const copiedNext = await expect.poll(async () =>
    page.evaluate(() => navigator.clipboard.readText()),
  );
  await copiedNext.toBe(
    "Extract checkout follow-ups: prompted next - Rerun checkout-flow.spec with retry trace (summary)",
  );

  await page
    .getByTestId("canvas-agent-extracted-item")
    .filter({ hasText: "Auth fixture token expired" })
    .getByRole("button", { name: "Request proof for Blocker" })
    .click();
  await expect
    .poll(async () =>
      page.evaluate(() => {
        ...
        const request = agent?.workstream?.inputQueue?.find(
          (item: { label?: string; text?: string }) =>
            item.label === "Request proof",
        );
        const blocker = agent?.workstream?.cockpitObjects?.find(
          (item: { text?: string }) =>
            item.text === "Auth fixture token expired",
        );
        return {
  ...
  await page
    .getByTestId("canvas-agent-extracted-item")
    .filter({ hasText: "Persist retry evidence" })
    .getByRole("button", { name: "Accept Task" })
    .click();
  await page
    .getByTestId("canvas-agent-extracted-item")
    .filter({ hasText: "npm test -- checkout-flow.spec passed" })
    .getByRole("button", { name: "Dismiss Evidence" })
    .click();
  await expect
    .poll(async () =>
      page.evaluate(() => {
        ...
        return (agent?.workstream?.cockpitObjects ?? []).map(
          (item: { text: string; reviewState: string; status: string }) =>
  ...
  await expect(
    page
      .getByTestId("canvas-agent-extracted-item")
      .filter({ hasText: "Evidence · dismissed" }),
  ).toContainText("npm test -- checkout-flow.spec passed");
