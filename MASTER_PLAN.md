...
| TC-043     | Scroll up dead on agent terminals (Codex+Claude): root cause = mouse-report stole the wheel even on the PRIMARY screen, where the scrollback lives in our grid and the inline agent ignores wheel — so scroll-up black-holed. Fix: mouse-report owns the wheel only on the alternate screen; primary-screen wheel → our history. Verify live (relies on the agent being primary-screen; Claude is) | P1       | FIX LANDED        | TC-042                  |
...
| TC-045     | Codex (alt-screen TUI) renders BROKEN in our canvas terminal: rows are blanked/the layout is fragmented, leaving a large empty band — the content IS there and renders perfectly in a real terminal outside TermFleet, so this is OUR rendering pipeline mangling a full-screen TUI (suspect: TC-037 reflow-on-grow fragmenting alt-screen, or diff/alt-screen render). NOT an empty-state issue | P1       | IN PROGRESS       | TC-037, TC-017          |
| TC-046     | Terminal copy/paste shortcuts unreliable: Ctrl+Shift+C and Ctrl+Shift+V do not consistently copy the selection / paste from the clipboard in the canvas terminal                                                                                                     | P1       | IN PROGRESS       | TC-017                  |
| TC-047     | Terminal rendering REGRESSION (P0): alt-screen TUIs fragment into "wreckage" and the black-bottom returns, on typing — introduced by TC-037 reflow-on-grow + TC-042 inner-column shrinking alt-screen grids. Add a per-terminal debug HUD (mode/layout/sizes) to see what breaks, then fix (freeze alt-screen, never reflow/shrink it) | P0       | IN PROGRESS       | TC-037, TC-042          |
