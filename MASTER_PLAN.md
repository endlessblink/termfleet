...
Update 2026-09-23 19:10 (claude): took over per Noam. Local work committed (e65bd38), then 60bacc0 / 2b8538d / 3e6ce5a / 644b0cd fixed 12 of 19 map failures (empty sidebar pre-daemon, instant-closing map rename, map opening below readable zoom, camera moving with a visible card, shell placeholder overriding linked task/running command, 5s placeholder hold). Remaining 7: box-select pan, note drag, Waiting filter count, running-command Now line, fixed task-row height, task picker list, token-pressure header. `tests/cockpit-row-stability.spec.ts` has 9 pre-existing failures (not a CI gate) → known issues.
...
Progress 2026-09-26 (claude): TF-059 — Claude Code panes "broke up" on scroll-up. Claude's `tui: fullscreen` mode draws on the primary screen with any-event mouse tracking; its screen clears stack stale frames into grid history, and TermFleet scrolled that history instead of giving Claude the wheel. New mouse-motion wire flag hands wheel + PageUp/Home/End to the app, a view parked in history snaps back first, and GTK's Shift+wheel (delivered as deltaX, previously scrolled DOWN) now reaches history. Commits 663a7d0 + dc1c9de; dock release 960ef28d6035. Proof: `cargo test --lib`, `playwright test terminal-mouse terminal-viewport`, `npm run verify:fullscreen-app-wheel` (real app, private Xvfb, network-free stand-in, screenshots). Issue stays `verifying` until Noam confirms in a real Claude pane after relaunch.
...
> the full Canvas2D suite (75/75), terminal reliability, restart/restore,
> daemon latency (p95 1.4 ms, max 1.4 ms), and standalone daemon cold-restore
> smoke. The canonical release gate emitted `TERMFLEET_RELEASE_CHECK_OK gui=1`.
> Fresh AppImage and deb artifacts were built; clean Debian testing containers
> installed and ran both formats. The promoted installed binary passed checksum
> verification, restart smoke, and the installed gamification visual gate with
> no external terminal processes. AppImage SHA-256 is
> `368be5fad8d0f06a11419497366774831037c38fc419ec74f394592134ca769c` and deb
> SHA-256 is `64f15f62fa4481ea0e8ca8d1fbaea00169b9ac4fea3c6acaeaf765d7a3d94b2d`.

> **2026-08-13 event-only profile and installed visual gate:** Promoted the
> event-only progression to profile v6 with a release-identity suffix, so a new
> packaged binary cannot reuse another release's local score. First launch marks
> existing work as observed without awarding it; later durable events remain
> rewardable. The bounded Workstream panel and runtime-detected dock interaction
> gate passed with a real Rank 1 · 0 Ops screenshot, Escape close proof, release
> executable identity, and no external terminal processes.

> **2026-08-13 event-only progression correction:** The prior implementation
> converted every completed task visible on first launch into points, causing
> unexplained Level 4/5 jumps. Profile v5 now records a trust horizon and ignores
> pre-existing events on initialization; later durable events remain rewardable.
> The panel is bounded to one Workstream quest, one latest badge, and one next
> badge, with the 10-minute → 30-minute → 3-hour stage text generated by the same
> engine. Focused event tests and the browser panel tests pass; installed visual
> verification remains pending until this change is promoted.

> **2026-08-13 PTY survival follow-up:** Locked compatible daemon rebuilds and
> fresh-daemon requests to reuse the canonical PTY owner instead of replacing it;
> the focused Rust regression, `npm run verify:map-terminals`, installed-release
> verification, and `git diff --check` passed. `npm run build` remains blocked by
> the unrelated dirty gamification panel's unused `alternatives` variable.

> **2026-08-13 contaminated-profile reset:** The prior v3 profile could survive
> every restart and return the user to Level 5. The redesigned economy now uses
> a fresh v4 local profile and explicitly rejects v3 data; workspace and terminal
> state are preserved. Added regression coverage for the exact 925-point failure.

> **2026-08-13 meaningful quests correction:** Replaced the misleading recovery
> badge with a real consecutive-work quest: keep three live terminals carrying
> tracked work continuously for 10 minutes. The timer displays elapsed/target,
> idle terminals do not count, and any interruption resets it to 0:00. Added
> plain-language command quest copy and 16 focused tests; Impeccable design gate,
> build, typography, installed release, and visual desktop gate passed. Promoted
> release `143b1a21c60a-7fabde0142d8`, SHA-256
> `7fabde0142d88150402d6ba0d6b7387856f05bd40b50cb9c7ddcd25f218d34bb`.

> **2026-08-13 canonical daemon ownership hardening:** Desktop launches now
> reuse the canonical per-user daemon across compatible builds and refuse to
> replace it for fresh or incompatible requests. Daemon request paths validate
> ownership before connecting, and the desktop PTY hook fails visibly instead
> of falling back to an embedded second owner. Fresh evidence: 167 Rust unit
> tests, `daemon_survival`, `cargo check`, `npx tsc --noEmit`, `npx vite build`,
> `npm run verify:map-terminals`, and `git diff --check` passed. Installed
> release verification is UNVERIFIED because an older long-running Tauri
> release build already owns the shared build resources.

> **2026-08-13 reward pacing and motion correction:** Slowed level thresholds to
> 100, 300, 750, 1500, 3000, and 6000 points, reduced successful command rewards
> to 5 points, removed recovery celebrations, and removed all floating reward
> toasts from the cockpit. The panel now uses a restrained teal/amber
> complementary pair and one purposeful reveal animation with reduced-motion
> support. Added an Impeccable-enforced gate that loads the Operate, Colorize,
> Animate, and Craft Floor guidance before the installed visual gate; 14/14
> gamification tests and the installed visual inspection passed.

> **2026-08-13 visual acceptance gate:** Added a fail-closed installed-desktop
> gate that refuses to run over an existing visible TermFleet window, proves the
> visible window executable matches the promoted release, clicks the measured
> header trigger, captures the opened panel, rejects the retired noisy drawer by
> OCR, and verifies Escape closes it. Independent inspection confirmed the real
> installed panel shows one primary challenge and the compact badge path.

> **2026-08-13 compact gamification correction:** Replaced the overwhelming
> progress drawer with one next challenge, at most two alternatives, one latest
> badge, and one next badge. Removed the default metrics/receipt ledger, made
> unavailable actions explicit, made reward toasts click-through and temporary,
> and bumped the local profile to v3 so the old level-2 history is not replayed
> after restart. Focused coverage passed 12/12; build, map, typography, diff,
> installed release, and installed restart smoke passed. Promoted release
> `bc556c97b098-867a848aba49`, SHA-256
> `867a848aba49a83e3d98d048972f66288b6ecd23ba0d8d8ff6720e334a1392ef`.

> **2026-08-13 gamification loop rebuild:** Reviewed and strengthened the
> reusable gamification skill so it requires a complete `choose -> focus ->
> complete -> explain -> choose` loop, not just ethical principles or badge
> definitions. The panel now presents one next move, lets the user choose among
> real missions, focuses an existing TermFleet tab/pane without spawning fake
> terminals, and explains completion evidence. Focused coverage passed 10/10;
> build, map, typography, diff, installed release, and installed restart smoke
> passed. Promoted release `bc556c97b098-329139f008f5`, SHA-256
> `329139f008f57be9c1384b6952550f001a42d98c439d4b336eafcb9fb5331961`.

> **2026-08-13 achievement clarity correction:** The progress panel no longer
> gives equal weight to noisy missions, receipts, and locked badges. It now shows
> `Achievements · earned/total`, a plain-language `What you have achieved` list
> with `Because: <real TermFleet evidence>`, and only the next two locked items
> with `Do this: <clear criterion>`. Live reward feedback says `Achievement earned`
> first and includes the exact evidence. Focused gamification coverage passed 9/9;
> build, map, typography, diff, installed release, and installed restart smoke
> passed. Promoted release `bc556c97b098-347cd65c1006`, SHA-256
> `347cd65c100660cc65e99a4d85470bf6109d76e4c4cd5634990bb09bc9eb1c19`.

> **2026-08-13 gamification redesign:** Replaced the counter-based reward slice
> with a versioned, local event system tied to real TermFleet behavior. Completing
> a tracked goal earns a contextual receipt, successful command-backed activity
> earns a receipt, terminal recovery is visible without meaningless points, and
> parallel work is a mission/achievement based only on active tracked workstreams.
> The panel now exposes three real missions, recent receipts, exact next actions,
> contextual achievements, explicit reset, v1 migration, and event-based animation.
> Created `.agents/skills/termfleet-gamification/` with research, design rules,
> eval prompts, and release gates. Focused coverage passed 9/9; build, map,
> typography, diff, installed release, and installed restart smoke passed.
> Promoted release: `bc556c97b098-914512fb0ec2`, SHA-256
> `914512fb0ec20391ab96227d9aa6de69ddc8e7994d8d4441adc476cdb7697a26`.

> **2026-08-12 Connect-terminal live verifier correction:** The isolated map
> verifier now closes the reset-state Workspace Map note, uses a deterministic
> Connect click fallback with coordinate overrides, refocuses the rendered
> terminal after the click, submits the probe, and compensates for Xvfb dropping
> the first synthetic character. The real PTY proof passed with
> `MAP_CONNECT_PTY_WRITE_OK`; the focused Connect regression, map source checks,
> frontend build, and `git diff --check` also passed.

> **2026-08-12 reward lifecycle correction:** Startup hydration no longer emits a
> progress reward, toast dismissal is no longer cancelled by frequent tab updates,
> and a persisted reset marker prevents stale desktop windows from restoring old
> progress. Focused gamification coverage passed 9/9; build, map verifier, installed promotion,
...
> injection in `pty.rs`, atomic-write fix, and a dev-only cockpit-state capture tool). The
> render path works (titles show real tasks again). BUT per-terminal isolation is **still
> dormant**: all same-folder terminals share ONE cwd-keyed sidecar, so they show the SAME
> task/list. Confirmed root cause: the running PTY daemon predates the injection, and a normal
> app restart **reuses** it — only `--fresh-daemon` or a PC reboot replaces it.
>
> **The one action that activates everything** (per-terminal task + list, capture tool, fresh
> JS) — run from a normal terminal in this repo, NOT inside the cockpit:
>
> ```
> ./run-native-vte-dev.sh --fresh-daemon
> ```
>
> (⚠️ cold-restores running shells. A PC reboot + normal launch does the same.)
>
> **Then verify, in order:**
>
> 1. In a cockpit pane: `env | grep TERMFLEET_PANE_ID` → prints `terminal-<tab>-<pane>`.
> 2. `ls ~/.local/share/terminal-workspace/agent-status/pane-*.json` → one file per terminal.
> 3. Two terminals in the **same folder** doing different work → each shows its **own** title
>    AND activity/task list (no shared/stomped status).
> 4. `npm run cockpit:snapshot` → table of every terminal's shown-title · source · real-task,
>    flagging ⚠ MISMATCH where a title is a command scrape instead of the real task.
>
> **If still wrong after a clean relaunch:** use `npm run cockpit:snapshot` to see which source
> feeds each bad title (real-task / narration / durable-activity / neutral) — the faithful view
> we built so we stop guessing. Then mark TC-035 verified and move to #5 (restart-survival) /
> #6 (close-cleanup). Delete this banner once verified.
...
| TC-059     | Repo-local regression planning and verification skills now turn repeated bugs into failure-surface guards and require sequential proof on the real runtime surface before completion or merge. The planner distinguishes pure logic, browser rendering, GTK/WebKit input, daemon persistence, and packaged-app failures; the verifier keeps runtime-only coverage partial until the live or installed surface is exercised. Follow-up 2026-07-21: both skills explicitly allow implicit invocation, and `AGENTS.md` requires the planner→verifier workflow automatically with a direct-file fallback when skill discovery is unavailable—users never need to name a skill or start another session. Proof: both skills passed `quick_validate.py`; `npm run build` and `git diff --check` passed; independent prompts that did not mention either skill still required a live GTK/WebKit/PTY paste guard and rejected a task-title merge without real desktop proof.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | P1       | DONE (2026-07-21)                                 | TC-052, TC-054         |
| TC-060     | The cockpit task line is always true, always current, always readable — the operator's four invariants: never blank (`Task not captured` must never render), never invented (every word is the agent's own, the operator's own, or a fixed template over a verified fact), never stale (the line dies when its turn ends), always plain (a non-developer can read it). Floor rule: when nothing better is knowable, show the operator's own request; for an agent-less shell, show what that terminal is actually doing. Root causes measured 2026-07-22: the Task row accepts only agent-declared text (present in 79/215 panes) while the templated activity string exists for 196/215 and is forbidden; quality-gate rejection renders the same string as "no data"; the 30-minute sidecar TTL lets finished tasks linger; shell panes have no derivation path; nothing reads the vendor session transcripts. New evidence: Claude and Codex both write hook-free on-disk session records (`~/.claude/projects/*.jsonl` incl. `ai-title` + `last-prompt`; `~/.codex/sessions/**/rollout-*.jsonl` incl. `task_started`/`task_complete`), Claude keeps a pid-keyed live registry at `~/.claude/sessions/<pid>.json`, and the process table maps every pane to its conversation id — together covering 209/215 panes vs 79 today, with shells covered by the process route. Design: a single daemon-owned resolver with a six-rung provenance ladder, per-rung expiry driven by real turn boundaries, and `npm run verify:task-line` sweeping every live pane as the release gate. Spec: `docs/superpowers/specs/2026-07-22-reliable-task-line-design.md`; plan: `docs/superpowers/plans/2026-07-22-reliable-task-line.md`. **Shipped 2026-07-22 (core):** the app now reads both vendors' own session records, resolves a seven-rung ladder that cannot return blank, and threads it through the header view model into all three surfaces; Rust gained an allowlisted transcript tail-reader and a pane foreground-command reader. Proof: 6 cargo tests, 22 frontend tests, `npm run verify:task-line` sweeps all 17 live panes with zero violations (agent-said 6, session-title 4, operator-request 3, current-tool 2, declared 1, shell-state 1 — every one previously `Task not captured`), `npm run verify:task-identity` 17/17, `npm run build` and `npm run doctor` pass with both vendor drift probes green. **Plumbing fix 2026-07-28 (operator report "tasks still don't appear", four screenshots):** the ladder and the records were both fine — a replay of the real resolver over all 238 live status files plus the vendor records produced a real line for every one of them, while the cockpit showed `No task declared` on idle Claude panes, a running Codex pane, and a pane that was mid-question. Four pieces of plumbing lost it: (1) `summarizeAgentStatus` attached the line only to its no-endpoint return, so the endpoint and error paths dropped it; (2) `statusPollLoop` — the ONLY loop that visits panes whose runtime is off screen — computed the line and never applied it, and `statusPollProjectionChanged` could not see it anyway, so every card that was not the one on screen had no line at all; (3) `persistedTerminalSnapshot` dropped it, so the first paint after every relaunch started empty and the header's own factless re-resolve printed the placeholder; (4) `MagicCanvas`/`SplitPane` read only the terminal's line while agent-kind tabs store it on the workstream. Added on the operator's request: a `pending-question` rung that states what the agent is asking them (`Waiting on your answer: how should the blur over the pixelation be controlled`, from Claude's own `AskUserQuestion` record, dropped again as soon as a later tool call shows it was answered) and a `recent-activity` rung so the agent's own newest note beats admitting nothing is known. Proof: `tests/task-line-live-records.spec.ts` (new — drives the real summarizer with real readers for BOTH on-disk sources; 238/238 panes render a real line, and it fails if a readable record yields the placeholder), `tests/task-line-plumbing.spec.ts` (new — 10 checks pinning each hole above), 64/64 of the existing task-line/identity/poll/transcript/audit suites, `npm run build`, and `npm run doctor` reporting `Task line coverage: 21/21`. New probe: `npm run cockpit:why`. **Readable goal 2026-07-28 (operator: "I need a user readable main goal... if I come a week from now I have no idea what this is about"):** two live cards read `exercise-demo-gif-pipeline` and `fix-cockpit-task-display`. Verified origin: both are Claude's OWN session title for those conversations (no Codex record contains either), and the vendor rewrites that title as a session runs — the same exercise session earlier carried the readable "Find free exercise visualization tool for fitness bot" and the app took the newest. Meanwhile the best answer to "what is this about" was unread: the operator's opening request lives at the START of the record and the app only ever read the tail (which carries "/done", "go"). Now: Rust gained a head reader (`session_transcript_head_read`, 64 KiB, same allowlisted dirs and id validation as the tail); a new `opening-request` rung leads the ladder under `declared`; the clearest session title wins over the newest; a slug is spaced into words (`Fix cockpit task display`) and `audit:panes` fails on any slug-shaped Task row; the composer's `❯` glyph is stripped; and the card's Task row is a FIXED two-line box (the terminal-jiggle contract in `tests/cockpit-row-stability.spec.ts` was deliberately re-pinned from "one line" to "two lines at a fixed height" — the jiggle came from a row whose height varied with the text; the fleet list stays one line). Fitting now allows 150 characters with the 200-character paste ceiling unchanged. Proof: 49/49 `verify:task-line`, `audit:panes` green with the new no-slug rule, 149/151 across the header suites (one pre-existing failure from a concurrent session), 7/7 Rust tc060 tests including a new head-reader bound, and `cockpit:why` on the live records shows `opening-request` and de-slugged titles in place of the slugs. **Remaining:** delete the per-project hardcoded outcome strings, feed the foreground command into the running-command rung, and demote the top rungs from live turn-end events end to end.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | P0       | IN_PROGRESS (core shipped 2026-07-22)             | TC-035, TC-052, TC-054 |
...
the durable workspace layout is restored, then exits after the restored surface
...
### TC-062: Drawing board panel on the operations map

**Priority:** P2
**Status:** Spike done (2026-07-25) — pending operator judgement of the look in the live app

A sketch surface next to the terminals, as a map node rather than a separate tab, so
arrows and notes can sit beside the panes they describe. Excalidraw (MIT, ships free
in a packaged app) over tldraw, whose SDK needs a paid licence to lose its watermark.

Two things make a pointer-driven editor survive a pan/zoom surface:

- **Level of detail.** Below `READABLE_TERMINAL_ZOOM` the board renders a cached still
  image of the last saved scene, the same trade a terminal node makes with its
  character preview. The editor bundle is only imported once a board is close enough
  to draw on, so flying past boards costs nothing.
- **Zoom compensation.** The map scales its contents with a CSS transform, but a
  drawing tool reads the pointer straight off the screen. The live board is laid out at
  `size * mapZoom` and counter-scaled by `1 / mapZoom`, so one CSS pixel inside the
  editor is one screen pixel at any map zoom; the map's zoom is handed to the editor's
  own zoom instead, so strokes still grow with the map.

Boards are plain `.excalidraw` JSON under
`~/.local/share/terminal-workspace/boards/<nodeId>.excalidraw` — deliberately a format
an agent in one of the terminals can write.

Two bugs the verifier caught, both in the save path:

- Saving read the scene back from the editor at flush time, and a flush can land while
  the editor is being torn down (board scrolled off, map zoomed out). A torn-down
  editor reports an empty scene, which overwrote the drawing. The scene is now captured
  at change time.
- The seed data was memoised, so a board coming back from the zoomed-out preview
  re-seeded the editor with whatever it held on first load — empty — and then saved
  that over the real drawing.
...
- `npx tsc --noEmit` clean, `npm run build` clean (the editor is code-split, so it is
  not in the startup bundle).
- `verify-map-terminals` 2 FAILs and `verify-typography` 45 FAILs — identical to the
  pre-change baseline measured in a HEAD worktree.
...
- **T-TASKS — faulty task descriptions / task lists (DONE):** the TASKS panel showed
  runner/verify/build _outcomes_ ("Terminal summary visual checks failed", "Frontend
...
- **T9 — input reliability — UNCONFIRMED (no fix; agent false-positives):** traced both
  ...
  - _Ctrl+C "dropped":_ `queue()` appends to `pendingInput` (`daemonInputQueue.ts:108`)
    **before** the `shouldFlushImmediately && flushTimeout` branch flushes (110-117), so a
    ...
    the PTY.
  - _Input-listener leak:_ `markInputListenerActive` deletes the **current** entry during
...
- **E — renderer artifacts (cursor ghost trail ALREADY FIXED + guarded; DPR blur
  out-of-scope):**
  - _Cursor ghost trails:_ already fixed in `gridBuffer.ts` (85-92) — `cursorMoved`
  ...
  - _DPR re-atlas blur:_ the atlas is created in `TerminalCanvas.tsx` (concurrent session
...
## 2026-08-13 — Make dock restart restore wait for the terminal keeper

The dock child previously launched saved-session restore before the desktop app
had created its canonical daemon. On normal startup the restore helper could
therefore time out, leaving the cockpit without its saved terminals; the app
could also exit during that race. The launcher now starts the app first, waits
for the canonical daemon socket, and only then starts the restore helper.

**Evidence:** `python3 -m unittest tests/test_installed_release.py -v` passed
14/14; `npm run release:install` completed and promoted the updated dock
launcher; `scripts/verify-installed-release.sh` passed; and
`scripts/verify-installed-restart-smoke.sh` passed with one visible TermFleet
