...
//!
//! Nikud / combining-mark shaping is intentionally out of scope here
//! (TC-018 Stage 3 fast-follow); this increment only reorders columns.
...
pub fn order_row(cells_text: &[Option<char>], _base: BaseDir) -> RowOrder {
    ...
    if !cells_text
        .iter()
        .any(|c| c.map_or(false, is_strong_rtl))
    {
...
mod tests {
    ...
    fn cursor_column_maps_logical_to_visual() {
        ...
        assert_eq!(order.log_to_vis[4], 7, "first Hebrew char goes rightmost of its run");
